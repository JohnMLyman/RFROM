...
mask_gil=~isfinite(squeeze(heat(:,:,1,2)));

mask_gil=[mask_gil(lon>360,:) ; mask_gil(lon<360,:)];
mask_gil=repelem(mask_gil,4,4);

mask_gil_bad=ones(1440,720);

mask_gil_bad(:,101:680)=mask_gil;
mask_gil_bad=logical(mask_gil_bad);
clear lon lat pres time mask_gil
...
for ilayer=startlayer:endlayer
    ...
    % apply mask that looks like gilson

    arwj(mask_gil_bad)=nan;
...
save ([path_tree,'test_tree_curve_yearly_7day_2000_yearly_new_cycle_mask.mat'], 'tgrid', 'ht_curve', 'basin_curve', 'layer_curve');
