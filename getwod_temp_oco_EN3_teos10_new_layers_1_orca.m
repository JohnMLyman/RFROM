function getwod_temp_oco_EN3_teos10_new_layers_1_orca(OcoSetUp)
...
%   if exist(sal_file,'file');load(sal_file);oclnum_sal=oclnum;end
  ...
  [coords,dt,time,nprof,qual,fflag,ptype,pinfo,oclnum,temp]=read_EN3_file_1_new(ifile,path_EN4_in);
                ...
                sal_junk=ones(length(press_junk),1).*35.0;
                        ...
                        for ilayer=2:length(layer_bounds)
                             ...
                             if max_depth_junk >= layer_bounds(ilayer)
                                ...
                                [temp_junk]=compute_depth_temp_depth_range_teos10_2016_orca(data,temp_junk,coords(j,:),layer_bounds(ilayer-1),layer_bounds(ilayer));
                                [temp_junk2]=compute_depth_temp_depth_range_teos10_2016_orca(data,temp_junk2,coords(j,:),0,layer_bounds(ilayer));
                        ...
                        for ilayer=2:length(layer_bounds)
                            ...
                            if ilayer~=length(layer_bounds)
                                ...
                                if_statement_name=[if_statement_name,'isfinite(temp_',num2str(layer_bounds(ilayer-1)),...
