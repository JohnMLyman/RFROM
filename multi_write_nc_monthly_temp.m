function []=multi_write_nc_monthly_temp(TreeSetUp)
...
path_mat_erddap=[path_ERDDAP,'matlab\'];
path_nc_erddap=[path_ERDDAP,'netcdf\'];
...
%                load(file_name_mat_nc,'ht_estimate_mon','time_1950','lon_tpx','lat_tpx')
               [temp_estimate_mon,time_1950,lon_tpx,lat_tpx]=load_temp_estimate_mon(file_name_mat_nc);
               ntime=length(time_1950);
               nlon=length(lon_tpx);
               nlat=length(lat_tpx);
               npressure=length(mean_pressure);
               temp_estimate=nan(nlon,nlat,npressure,ntime);
               temp_estimate(:,:,1,:)=temp_estimate_mon;
...
%                        load(file_name_mat_nc,'ht_estimate_mon')
                       [temp_estimate_mon,~,~,~]=load_temp_estimate_mon(file_name_mat_nc);
                        temp_estimate(:,:,ilayer-1,:)=temp_estimate_mon;
               ...
               if imonth>=10
                      file_name_nc= [path_nc_erddap,'RFROM_TEMP_',num2str(iyear),'_',num2str(imonth),'.nc'];
                   else
                      file_name_nc= [path_nc_erddap,'RFROM_TEMP_',num2str(iyear),'_0',num2str(imonth),'.nc'];
               end
    
               write_netcfd_cf_temp_pressure_mon_single(temp_estimate,time_1950,lon_tpx,...
...
function [temp_estimate_mon,time_1950,lon_tpx,lat_tpx]=load_temp_estimate_mon(filename)


    load(filename, 'temp_estimate_mon','time_1950','lon_tpx','lat_tpx')
