function []=multi_add_seasonal_cycle(TreeSetUp,path_new_tree,tree_model,start_year_file,end_year_file)
...
%%
tree_model_withcycle=[tree_model,'_withcycle'];
...
%%
if not(exist([path_new_tree,'withcycle'],'dir'))
    mkdir([path_new_tree,'withcycle'])
end
%%
parfor year_load=time_ssh_load
    ...
    for ilayer=2:nlayer
        ...
        [ht_estimate,time_aviso,lon_tpx,lat_tpx]=load_ht_estimate([path_new_tree,tree_file_name_out,'_split_7day.mat']);
        s_ht=size(ht_estimate);
        nlon_tpx=s_ht(1);
        nlat_tpx=s_ht(2);
        ntime_tpx=s_ht(3);
        [ht_cycle,ht_mean,~]=load_cycle([path_tree,tree_model_file_name_season,'_seasonal_cycle_split.mat']...
            ,nlon_tpx,nlat_tpx,ntime_tpx,time_aviso,center_year);
        ht_estimate=ht_estimate+ht_cycle+ht_mean;

        parsave_tree_year([path_new_tree,'withcycle\',tree_file_name_out_withcycle,'_split_7day.mat'] ,ht_estimate,lon_tpx, lat_tpx,time_aviso)
    end
end

end
function [ht_estimate,time_aviso,lon_tpx,lat_tpx]=load_ht_estimate(filename)
