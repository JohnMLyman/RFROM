...
mean_depth_bnds=[layer_bounds(1:end-1); layer_bounds(2:end)];
   ...
   for iyear=year_start_nc:year_end_nc
       for imonth=1:12
           ...
           file_name_mat_nc=[path_mat_nc,tree_file_name,'_NOCYLE_',num2str(iyear),'_',num2str(imonth),'.mat'];
%            file_name_nc= [path_nc,'RFROM_OHCA_',num2str(iyear),'_',num2str(imonth),'.nc'];
           ...
           if exist(file_name_mat_nc,'file')
               ...
               for ilayer=3:nlayers
                   ...
                   layer_name=[num2str(layer_bounds(ilayer-1)),'_',num2str(layer_bounds(ilayer))];
                   tree_file_name=[tree_model_file_name,'_',layer_name];
                   file_name_mat_nc=[path_mat_nc,tree_file_name,'_NOCYCLE_',num2str(iyear),'_',num2str(imonth),'.mat'];
               ...
               if imonth>=10
                      file_name_nc= [path_nc,'RFROM_OHCA_NOCYCLE_',num2str(iyear),'_',num2str(imonth),'.nc'];
                   else
                      file_name_nc= [path_nc,'RFROM_OHCA_NOCYCLE_',num2str(iyear),'_0',num2str(imonth),'.nc'];
               end

    
               write_netcfd_cf_heat_depth_mon_single_no_cycle(ht_estimate,time_1950,lon_tpx,...
