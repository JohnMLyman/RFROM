...
load('C:\Users\jlyma\OneDrive - University of Hawaii\data\topo_tpx_new.mat','topo_tpx_new','lon_topo')
 topo_tpx_new=-1.*topo_tpx_new;

ilayer=2;
nlayers=length(layer_bounds);
layer_name=[num2str(layer_bounds(ilayer-1)),'_',num2str(layer_bounds(ilayer))];
name_year=num2str(min_year_fit);
...
% load([path_tree,tree_file_name,'_split_7day.mat'], 'lon_tpx' ,'lat_tpx','time_aviso')
 load ([path_tree_junk,'netcdf\sal\matlab\mean_sal_yearly\',tree_file_name_season,'_',name_year,'.mat'] ,'lon_tpx')
...
for iyear=min_year_fit:max_year_fit
    name_year=num2str(iyear);
    load ([path_tree_junk,'netcdf\sal\matlab\mean_sal_yearly\',tree_file_name_season,'_',name_year,'.mat'] ,'ht_estimate', 'lat_tpx','time_aviso')
...
%     end
     bad=bottom_depth<layer_bounds(ilayer-1);
     ht_estimate(:,bad,:)=nan;
   ht_out_total_layer=cat(3,ht_out_total_layer,ht_estimate(pos_lon,:,:));
 ...
 ht_out_total=cat(1,ht_out_total,ht_out_total_layer);

toc./60
end
clearvars ht_estimate
time_aviso=time_aviso_total;
ht_estimate_mean=permute(ht_out_total,[2 1 3]);
clear ht_out_total time_aviso_total
...
save(file_sum_name,'ht_estimate_mean','depth','lat_tpx','time_aviso','-v7.3')
