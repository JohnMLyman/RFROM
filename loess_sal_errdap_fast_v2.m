...
path_nc_erddap_sal=[path_ERDDAP_sal,'netcdf\',tree_prefix,'\',subdir,'\'];
path_nc_erddap_temp=[path_ERDDAP_temp,'netcdf\',tree_prefix_temp,'\',subdir,'\'];

path_nc_erddap_sal_loess=[path_ERDDAP_sal,'netcdf_loess\',tree_prefix,'\',subdir,'\'];
% path_nc_erddap_temp_stable=[path_ERDDAP_temp,'netcdf_stable\'];


if ~exist(path_nc_erddap_sal_loess,'dir')
    mkdir(path_nc_erddap_sal_loess)
end
...
%    parfor iyear=year_start_nc:year_end_nc
%        for imonth=1:12
%       
         imonth=months(imod);
            iyear=years(imod);
...
%            file_name_nc= [path_nc_erddap,'RFROM_OHCA_',num2str(iyear),'_',num2str(imonth),'.nc'];
               ...
               if imonth>=10
                      file_name_nc_sal= [path_nc_erddap_sal,'RFROMV2_SAL_',num2str(iyear),'_',num2str(imonth),'.nc'];
                      file_name_nc_temp= [path_nc_erddap_temp,'RFROMV2_TEMP_',num2str(iyear),'_',num2str(imonth),'.nc'];
                      file_name_nc_sal_loess= [path_nc_erddap_sal_loess,'RFROMV2_SAL_LOESS_',num2str(iyear),'_',num2str(imonth),'.nc'];
                     

                   else
                      file_name_nc_sal= [path_nc_erddap_sal,'RFROMV2_SAL_',num2str(iyear),'_0',num2str(imonth),'.nc'];
                      file_name_nc_temp= [path_nc_erddap_temp,'RFROMV2_TEMP_',num2str(iyear),'_0',num2str(imonth),'.nc'];
                      file_name_nc_sal_loess= [path_nc_erddap_sal_loess,'RFROMV2_SAL_LOESS_',num2str(iyear),'_0',num2str(imonth),'.nc'];
