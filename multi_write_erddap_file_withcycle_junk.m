function []=multi_write_erddap_file_withcycle_junk(TreeSetUp)
...
for year_load=time_ssh_load
    ...
    [ht_estimate,lat_tpx,lon_tpx,time_aviso]=...
        make_vertical_year_junk(path_new_tree,tree_model,year_load,TreeSetUp);
    ...
    for imonth=1:12
        ...
        if ~isempty(time_1950)
            ...
            if imonth>=10
                  file_name_nc= [path_nc_erddap,file_prefix,num2str(year_load),'_',num2str(imonth),'_junk.nc'];
               else
                  file_name_nc= [path_nc_erddap,file_prefix,num2str(year_load),'_0',num2str(imonth),'_junk.nc'];
            end
            if var_type =='t'
