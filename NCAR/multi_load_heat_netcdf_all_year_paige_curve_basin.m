function [mean_depth,mean_depth_bnds,yr_out,ohca_curve_total]=multi_load_heat_netcdf_all_year_paige_curve_basin(years_load,TreeSetUp,latmin,latmax,basin_number)
...
ohca_curve_total=nan(nlayers,ntime,12);
mask_total=nan(length(lon),length(lat),10,12);
for basin_number=1:12
    ...
    mask_total(:,:,:,basin_number)=mask.*basin_mask;
...
for year_load=years_load
    ...
    for imonth=1:12
            ...
            for basin_number=1:12
                mask=squeeze(mask_total(:,:,:,basin_number));
               
                ohca_curve=ohca.*mask.*arw;
                ohca_curve=jnansum(ohca_curve,1);
                ohca_curve=squeeze(jnansum(ohca_curve,2));
                ohca_curve_total(:,start_ind:end_ind,basin_number)=ohca_curve;
            end
...
%     ohca_curve_total(:,:,basin_number)=ohca_curve_out;
