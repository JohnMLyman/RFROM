function [amp_annual_total,phase_annual_total,amp_semi_total,phase_semi_total,...
    amp_third_total,phase_third_total,slope_total,mean_total,model_total]=...
    make_seasonal_cycle_comp_NCAR_all(yr,ohca)
...
model_total=nans(nlon_tpx,nlat_tpx,ntime);

parfor ilat=1:nlat_tpx 
    for ilon=1:nlon_tpx 
%         for ilayer=1:nlayers
         jyin=squeeze(jnansum(squeeze(ohca(ilon,ilat,:,:)),1))';
         jtime=yr;
         good=isfinite(jyin);
         jyin=jyin(good)';
         jtime=jtime(good);
         model_fit_out=nans(1,ntime);

         if length(jtime)>percent_good_fit*ntime % only fit a seasonal cycle when there is 50% coverage
%              jyin=jyin-nanmean(jyin);

             [model_fit,amp_annual,phase_annual,amp_semi,phase_semi,amp_third,phase_third,slope_fit,mean_fit,model_err]=...
             ...
             model_fit_out(good)=model_fit;
             model_total(ilon,ilat,:)=model_fit_out;
%          end
         end
%                  
    end
