function []=write_netcfd_cf_heat_depth(ht_estimate,lon_tpx,lat_tpx,time_aviso,path_nc,...
    ohca_description,file_nc_prefix,year_start_nc,year_end_nc,depth_min,depth_max)
...
%     good=floor(time_aviso)==iyear;
    ...
    latitude=lat_tpx';
    longitude=lon_tpx';
    depth_bnds=[depth_min depth_max];
    ...
    mySchema.Dimensions(4).Name   = 'vertices';
    mySchema.Dimensions(4).Length = nv;
    ...
    depth_bnds_dimen.Name='vertices';
    depth_bnds_dimen.Length=nv;
    ...
    mySchema.Variables(5).Name='mean_depth';
    mySchema.Variables(5).Dimensions=mean_depth_dimen;
    mySchema.Variables(5).Attributes=mean_depth_att;
    mySchema.Variables(5).Datatype='double';
    mySchema.Variables(5).FillValue='disable';


        % depth_bnds
    mean_depth_bnds_att(1).Name='units';
    mean_depth_bnds_att(1).Value='meters';
    mean_depth_bnds_att(2).Name='Description';
    mean_depth_bnds_att(2).Value='The upper and lower bounds of vertical integration';
    ...
    mySchema.Variables(6).Datatype='double';
