...
% covert yearday to year and month
...
%%
yrf=floor(time_aviso);

junk_year=[yrf' ones(ntime_tpx,1) ones(ntime_tpx,1)];
d3=datenum(junk_year)';


% yr=yrf+(aviso_datnum-d3)./yeardays(yrf);
aviso_datnum=d3+(time_aviso-yrf).*yeardays(yrf);
dt=datevec(aviso_datnum');
month_aviso=dt(:,2)';
clearvars yrf d3 dt aviso_datnum

 v=VideoWriter([path_figs,'MLD_95_mon.avi']);
...
clf
    ...
    good_month=(month_aviso==iframe);
    iqr95=prctile(ht_estimate(:,:,good_month),95,3);
    corrhc=iqr95;
...
%     save 'OHCA_2019_tpx.mat' lon lat corrhc
    ...
    hold on
    m_coast('patch',[1 1 1]);
    t1=m_text(30,100,[' 95% for month ', num2str((iframe))],'fontsize',12);
