function []=make_model_stats_all_years_season_orca_novert_nosshsst_paige(TreeSetUp)
...
%         nbasin=length(model_all);
        ...
        %     load(file_big_model,'model_all')
            ...
            if ~isempty(Mdl)
                ...
                if ilayer==2
                    PredictorNames={'Year' 'Lon' 'Lat' 'SSH' 'SST' 'COS' 'SIN'};
                elseif ilayer<=ilayer_depth_use_sst
                      PredictorNames={'Year' 'Lon' 'Lat' 'SSH' 'SST' 'COS' 'SIN'};
                elseif ilayer<=ilayer_depth_use_ssh
                      PredictorNames={'Year' 'Lon' 'Lat' 'SSH' 'COS' 'SIN'};
                else
                    PredictorNames={'Year' 'Lon' 'Lat' 'COS' 'SIN'};
                end
               %             h.XTickLabel =PredictorNames;
                ...
                TreeJunk(ibasin,1).BasinName=model_all.name;
