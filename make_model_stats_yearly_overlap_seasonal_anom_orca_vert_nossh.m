function []=make_model_stats_yearly_overlap_seasonal_anom_orca_vert_nossh(TreeSetUp)
...
parfor ilayer=2:nlayer
    ...
    filejunk=[path_tree_junk,tree_model_file_name,'_model_',layer_name,'_junk.mat'];
    TreeJunk=[];
    ...
    for iyear_mod=start_year_mean:.5:end_year_mean
        ...
        file_big_model=[path_tree_junk,tree_model_file_name,'_model_',layer_name,'_',year_file_name,'_split.mat'];
        ...
        for ibasin=1:nbasin
            ...
            if ~isempty(Mdl)
                ...
                elseif ilayer<=ilayer_depth_use_sst
                      PredictorNames={'Year' 'Lon' 'Lat' 'SSH' 'SST' 'TEMP_layerup'};
                else
                    PredictorNames={'Year' 'Lon' 'Lat' 'TEMP_layerup'};
                end
...
for ilayer=2:nlayer
    ...
    for iyear_mod=start_year_mean:.5:end_year_mean
        iyear=(iyear_mod-start_year_mean)*2+1;
        for ibasin=1:nbasin
                    TreeStats(ibasin,ilayer,iyear).PredictorNames=TreeJunk(ibasin,iyear).PredictorNames;
                    TreeStats(ibasin,ilayer,iyear).PredictorImpotance=TreeJunk(ibasin,iyear).PredictorImpotance;
                    TreeStats(ibasin,ilayer,iyear).oobError=TreeJunk(ibasin,iyear).oobError;
                    TreeStats(ibasin,ilayer,iyear).LayerName=TreeJunk(ibasin,iyear).LayerName;
                    TreeStats(ibasin,ilayer,iyear).LayerTitle=TreeJunk(ibasin,iyear).LayerTitle;
                    TreeStats(ibasin,ilayer,iyear).BasinName=TreeJunk(ibasin,iyear).BasinName;
        end
...
file_stats=[path_new_tree,tree_model_file_name,'_all_stats_split.mat'];
