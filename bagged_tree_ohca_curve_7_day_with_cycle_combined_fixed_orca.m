function []=bagged_tree_ohca_curve_7_day_with_cycle_combined_fixed_orca(TreeSetUp)
...
for ilayer=startlayer:endlayer
    ...
    load([path_tree,tree_file_name_old,'_seasonal_cycle_expand_split.mat'],'ht_cycle')
...
%     end
   ...
   % mask out where there are anny missing values so ocean area is fixed
   %  day to day, varibiily is mostly do to ice.
   pos_fixed=repmat(~logical(min(isfinite(ht_out),[],3)),[1 1 ntime_tpx]);
   ht_out(pos_fixed)=nan;
...
save ([path_tree,'test_tree_curve_yearly_7day_2000_yearly_new_cycle_combined_2_fixed_orca.mat'], 'tgrid', 'ht_curve', 'basin_curve', 'layer_curve');
