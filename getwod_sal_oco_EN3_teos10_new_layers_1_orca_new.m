function getwod_sal_oco_EN3_teos10_new_layers_1_orca_new(OcoSetUp)
...
for ifile=1:nfile
    ...
    [coords,dt,time,nprof,qual,fflag,ptype,pinfo,oclnum,mdep,wnum,data]=read_EN3_file_1_sal_new_orca(ifile,path_EN4_in);
...
%         [data]=find_woce_abssal_new_layers_1_orca(data,coords,layer_bounds,path_OHCA_data_out);
    ...
    sal_junk_total=nan(nprof,n_player);
    %%
    ngroup_layer=4;
    if n_player>= 20
    ...
    for isublayer=1:ngroup_layer
        ...
        parfor player=start_sublayer(isublayer):end_sublayer(isublayer)  %change to parfor
            ...
            sal_junk=nan(nprof,1);
           
%             [temp_junk]=compute_depth_temp_depth_range_teos10_2016_tuna(data,temp_junk,coords,layer_bounds(player),layer_bounds(player+1));
            [sal_junk,temp_junk]=compute_depth_sal_press_range_teos10_2016_tuna(data,sal_junk,temp_junk,coords,layer_bounds(player),layer_bounds(player+1));

           temp_junk_total(:,player)=temp_junk;
           sal_junk_total(:,player)=sal_junk;
        end
    end
    temp_all{ifile}=temp_junk_total;
    sal_all{ifile}=sal_junk_total;
    dt_all{ifile}=dt;
 ...
 for ifile=1:nfile
    ...
    wnum_wod=wnum_all{ifile};
                         ...
                         for ilayer=2:length(layer_bounds)
                             ...
                             junk_sal=sal_all{ifile}(:,ilayer-1);

                             eval(['temp_',num2str(layer_bounds(ilayer-1)),'_',num2str(layer_bounds(ilayer)),'_wod(sprof:endprof)=',...
                                 'junk_temp;'])
                             eval(['sal_',num2str(layer_bounds(ilayer-1)),'_',num2str(layer_bounds(ilayer)),'_wod(sprof:endprof)=',...
                                 'junk_sal;'])
                         end
                        if ifile~=nfile
                         sprof=endprof+1;
