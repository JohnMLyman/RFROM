function [sst]=find_oisst_4_mean(lonp,latp,yr)
...
new_years=floor(min(yr)):floor(max_year);
...
lat_sst=double(ncread([path_oisst,'sst.day.mean.1990.nc'],'lat'));
lon_sst=double(ncread([path_oisst,'sst.day.mean.1990.nc'],'lon'));
...
time=double(ncread([path_oisst,'sst.day.mean.',year_name_min,'.nc'],'time'));
sst_anom_year=ncread([path_oisst,'sst.day.mean.',year_name_min,'.nc'],'sst');
...
for yr_value=load_years
  ...
  if exist([path_oisst,'sst.day.mean.',yr_name_next,'.nc'],'file')
      ...
      time_next=ncread([path_oisst,'sst.day.mean.',yr_name_next,'.nc'],'time');
