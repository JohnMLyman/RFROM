function []=make_heat_ME4OH_extra(OcoSetUp)
...
good_files=dir([path_ME4OH_profiles,'ofam3-jra55.all.EN.4.1.1.f.profiles.g10.*.update.extra.nc']);
...
for ifile=1:nfiles
    file=[good_files(ifile).folder,'\',good_files(ifile).name]
    [jdohc,~,dz,jlon,jlat,jdt]=read_profiles_ME4OH_extra(file);
    jnprof=length(jlon);
%     figure(1)
