...
time_grid=[min_year_maps_out-.5:.5:max_year_maps_out+.5];
...
tpx_maps=nans(nlon_tpx,nlat_tpx,nyears);
ht_maps=tpx_maps;
one_maps=tpx_maps;
...
depth_min_total=layer_bounds(1);
depth_max_total=layer_bounds(end);
...
%     httree_out=htdiff_tree_out+tree_out;
    ...
    ht_maps=nansum(cat(4,ht_out.*arwj,ht_maps),4);
    one_maps=nansum(cat(4,one_out.*(delta_depth./delta_depth_total).*arwj./arwj_all,one_maps),4);
    tpx_maps=nansum(cat(4,tpx_out.*arwj,tpx_maps),4);
    httpx_maps=nansum(cat(4,httpx_out.*arwj,httpx_maps),4);
