function [coords_tot,dt_tot,mdep_tot,npts_tot,press_0_tot,sal_0_tot,temp_0_tot, ... 
        time_tot,type_data_tot,wmo_tot,density_surface]= ...
        load_data_wmo_square_floats_wod(wmo,del_deg)
...
for i=1:length(wmo_squares)
   ...
   if exist(['w',num2str(wmo_squares(i)),'.mat']),
      eval(['load w',num2str(wmo_squares(i)),'.mat;']);
    wmo_squares(i)
    lon_junk=coords(:,1);
    lat_junk=coords(:,2);
    b=0;
    ...
    good_locations = find((lon_junk >= min_lon) & (lon_junk <= max_lon3) &...
        (lat_junk <= max_lat) & (lat_junk >= min_lat) & ...
        (mdep >= depth_min) & (npts >=points_min) & ...
        (type_data ~= 3));
  ...
  %  good_locations=find(finite(lon_junk));
    ...
    type_data=type_data(good_locations);
    ...
    type_data_tot=[type_data_tot',type_data']';
...
cd(current_dir);
clear temp_0 sal_0 npts type_data dt coords mdep time press_0
