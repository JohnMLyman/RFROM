function [hc,time,hc_one,one_curve,Area]=heat_curv_gen_mat_2012_new_un_topo_no_ice(file_name,depth_min,depth_max)
...
lor=lon_tpx;
lat=lat_tpx;
Area(i)=sum(arw(~isnan(corrhc)));
% compute the area average heatcontent across the globe.

one_curve(i)=nansum(arw(:).*corrhc_one(:));
