function mapclim_gen(file_allheat,file_clim)
...
% pick only data from 1993 through 2007
tpx=topex(:,1);
day=datenum(dt);
gind=find(day>=datenum(1993,1,1)& ...
	~isnan(tpx+heatanom));
%gind=find(day>=datenum(1993,1,1)&day<datenum(2007,1,1)& ...
%	~isnan(tpx+heatanom));
...
alat=lat;alon=ln;
save hregress alat alon alpha crl xvar
%return
cds=cd;
...
eval(['save ',file_clim,' gheatclim gheatclim2 ln lon lat map tpxmap levheat']) 
