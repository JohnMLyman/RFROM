...
function [syear]=sal_map_oco_2009_2010_6deg_2_pannel_multi_label_WOD09_gary(fyear)
...
sal=[lev_sal(end-40:end,:,:,:);lev_sal;lev_sal(1:41,:,:,:)];

lon_lev=double(lev_lon);
lat_lev=double(lev_lat);
sal_lev=nanmean(double(sal(:,:,1,9:12)),4);

lat2=repmat(lat_lev,[1,442])';

lon2=repmat(lon_lev,[1,180]);

lat21=lat2(:);
lon21=lon2(:);
sal21=sal_lev(:);

bad=find(~isfinite(sal21));
good=find(isfinite(sal21));

X=[lon21(good) lat21(good)];
q=[lon21(bad) lat21(bad)];
dt = DelaunayTri(X);
xi=nearestNeighbor(dt, q);

sal21(bad)=sal21(good(xi));
sal_lev_old=sal_lev;
sal_lev=reshape(sal21,[442 180]);
...
load 'surface_sal_jan_2011_2010_2011_6_deg_gary.mat' lat lon time one sal;
% 
% 
...
mean_lev_sal=interp2(lat_lev,lon_lev,sal_lev,lat_tpx,lon_tpx');
surface_sal=sal;
...
corrhc=interp2(lat,lon,surface_sal_2007,lat_tpx,lon_tpx')+mean_lev_sal;
corrhc(isnan(msk2(2:end-1,:)))=NaN;
...
corrhc(~isfinite(corrhc))=min_val+miss_val;
corrhc(~isfinite(land_mask))=NaN;
pcolor(lon,lat,corrhc')
caxis([min_val+miss_val max_val])
shading flat



%contour_mean_sal_no_label_WOD09
...
t1=text(60,54,['a) Sep-Dec '],'fontsize',11,'fontweight','bold');
t1=text(75,40,[num2str(fyear)],'fontsize',11,'fontweight','bold');
axis([30 390 -90 90])
axis equal
axis([30 390 -90 90])
a=gca;
...
% 
% 
% t1=text(55,54,['b) ',num2str(fyear),'-',num2str(syear)],'fontsize',11,'fontweight','bold');
% axis([30 390 -90 90])
% axis equal
% axis([30 390 -90 90])
% 
...
% %%eval(['print -dtiff -r600 -f1 /Users/johnlyman/figs/oco/Sal/oco_2008_oct_surface_sal_6deg_change_jan_2008_2007_2_pannel_test'])
 eval(['print -dpng -f1 /Users/johnlyman/figs/oco/Sal/oco_',num2str(fyear),'_jan_surface_sal_6deg_change_2010_2009_2_pannel_label2_gary2'])
 eval(['print -depsc2 -r600 -f1 /Users/johnlyman/figs/oco/Sal/oco_',num2str(fyear),'_jan_surface_sal_6deg_change_2010_2009_2_pannel_label2_gary2'])
