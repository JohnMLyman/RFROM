function [data_grid]=box_mean_map_mon2_test(lon_grid,lat_grid,data,cds,time,grid_bin)
...
%    % add in the depth 
  ...
  signal_to_noise=1;
   ...
   data_junk=nans(length(ii),4);
   data_junk(:,1)=data(ii,1);
   data_junk(:,2)=cds(ii,1);
   data_junk(:,3)=cds(ii,2);
   data_junk(:,4)=time(ii);
   ident=eye(length(ii));
