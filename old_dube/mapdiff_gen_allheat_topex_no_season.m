...
function mapdiff_gen_allheat_topex_no_season(file_name,fname_nc,clim_file)
...
eval(['load ' clim_file ' gheatclim2 lon lat ln'])
...
htdiff=htdiff(ii);day=day(ii,:);cds=cds(ii,:);htanom=htanom(ii);tpx=tpx(ii);tpx_no=tpx_no(ii);
...
eval(['save ',fname_nc,' htdiff tpx tpx_no yr coords htanom'])
nc_idl_save_hdata_gen_no_season(fname_nc);
