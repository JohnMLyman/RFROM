...
% leave in the annual cycle
htanom=ht-htclim;
save -append allheat htanom
clear i dmo lhc lon lat mo

% use topex without annual cycle removed
tpx=topex(:,1);
...
save hregress2 alat alon alpha c xvar
end % of skip
...
% The month in fraction of a year 

month_frac=[15,45,74,105,135,166,196,227,258,288,319,349]/365.25


% make grid variables
%tgrid=[1992.75:.25:2003.5];
%tgrid=[1990.5:.25:2005.0];
tgrid=1992+[month_frac(10),month_frac(11),month_frac(12)];

for iyear=1993:2004
    
       tgrid=[tgrid,iyear+month_frac];
...
save gridinfo mcds gind bind lon lat tgrid

% make list of indicies to be searched for each grid point with in two
% months of the reported time.  (ie. a box car filter with width of 4
% months

yr=(day-datenum(1992,1,1)+datenum(1950,1,1))/365.25+1992;
for i=1:length(tgrid)
 idx{i}=find(abs(tgrid(i)-yr)<=(60/365.25));
end

% spit out data (this part didn't work! (tgrid(i)-fix(tgrid(i))) was never
% == to month_frac!  This could be due to a rounding error)
for i=1:length(tgrid)
  ii=idx{i};
  fname=['hdata',num2str(fix(tgrid(i)))];
  if (tgrid(i)-fix(tgrid(i)))==month_frac(1),fname=[fname,'jan'];end
  if (tgrid(i)-fix(tgrid(i)))==month_frac(2),fname=[fname,'feb'];end
  if (tgrid(i)-fix(tgrid(i)))==month_frac(3),fname=[fname,'mar'];end
  if (tgrid(i)-fix(tgrid(i)))==month_frac(4),fname=[fname,'apr'];end
  if (tgrid(i)-fix(tgrid(i)))==month_frac(5),fname=[fname,'may'];end
  if (tgrid(i)-fix(tgrid(i)))==month_frac(6),fname=[fname,'jun'];end
  if (tgrid(i)-fix(tgrid(i)))==month_frac(7),fname=[fname,'jul'];end
  if (tgrid(i)-fix(tgrid(i)))==month_frac(8),fname=[fname,'aug'];end
  if (tgrid(i)-fix(tgrid(i)))==month_frac(9),fname=[fname,'sep'];end
  if (tgrid(i)-fix(tgrid(i)))==month_frac(10),fname=[fname,'oct'];end
  if (tgrid(i)-fix(tgrid(i)))==month_frac(11),fname=[fname,'nov'];end
  if (tgrid(i)-fix(tgrid(i)))==month_frac(12),fname=[fname,'dec'];end
  fname=[fname,'.txt'];
