...
for i=1:length(d)
  ...
  save(['e',d(i).name(2:end)],'bath','coords','dt','depth','mdep',...
	'nkept','npts','qual','temp','time','src','typ','isunk');
end
...
t=typ;s=src;isu=isunk;
load e1004
coords=[cds;coords];dt=[ddt;dt];time=[tm;time];bath=[bt;bath];
temp=[tmp;temp];npts=[np;npts];qual=[q;qual];mdep=[mdp;mdep];
typ=strvcat(t,typ);src=strvcat(s,src);isunk=[isu;isunk];
save e1004 coords dt time temp mdep npts qual depth nkept bath src typ isunk
!rm e1005.mat
end
...
tmp=[tmp;temp];np=[np;npts];q=[q;qual];bt=[bt;bath];isu=[isu;isunk];
t=strvcat(t,typ);s=strvcat(s,src);
load e1010
coords=[cds;coords];dt=[ddt;dt];time=[tm;time];bath=[bt;bath];
temp=[tmp;temp];npts=[np;npts];qual=[q;qual];mdep=[mdp;mdep];
typ=strvcat(t,typ);src=strvcat(s,src);isunk=[isu;isunk];
save e1010 coords dt time temp mdep npts qual depth nkept bath src typ isunk
...
temp=[tmp;temp];npts=[np;npts];qual=[q;qual];mdep=[mdp;mdep];
typ=strvcat(t,typ);src=strvcat(s,src);isunk=[isu;isunk];
save e1701 coords dt time temp mdep npts qual depth nkept bath src typ isunk
!rm e1702.mat
...
temp=[tmp;temp];npts=[np;npts];qual=[q;qual];mdep=[mdp;mdep];
typ=strvcat(t,typ);src=strvcat(s,src);isunk=[isu;isunk];
save e3312 coords dt time temp mdep npts qual depth nkept bath src typ isunk
...
temp=[tmp;temp];npts=[np;npts];qual=[q;qual];mdep=[mdp;mdep];
typ=strvcat(t,typ);src=strvcat(s,src);isunk=[isu;isunk];
save e5317 coords dt time temp mdep npts qual depth nkept bath src typ isunk
!rm e5318.mat
end
...
temp=[tmp;temp];npts=[np;npts];qual=[q;qual];mdep=[mdp;mdep];
typ=strvcat(t,typ);src=strvcat(s,src);isunk=[isu;isunk];
save e7011 coords dt time temp mdep npts qual depth nkept bath src typ isunk
!rm e7012.mat
...
temp=[tmp;temp];npts=[np;npts];qual=[q;qual];mdep=[mdp;mdep];
typ=strvcat(t,typ);src=strvcat(s,src);isunk=[isu;isunk];
save e7017 coords dt time temp mdep npts qual depth nkept bath src typ isunk
!rm e7018.mat
...
temp=[tmp;temp];npts=[np;npts];qual=[q;qual];mdep=[mdp;mdep];
typ=strvcat(t,typ);src=strvcat(s,src);isunk=[isu;isunk];
save e7317 coords dt time temp mdep npts qual depth nkept bath src typ isunk
!rm e7318.mat
...
temp=[tmp;temp];npts=[np;npts];qual=[q;qual];mdep=[mdp;mdep];
typ=strvcat(t,typ);src=strvcat(s,src);isunk=[isu;isunk];
save e7607 coords dt time temp mdep npts qual depth nkept bath src typ isunk
!rm e7608.mat
end
...
temp=[tmp;temp];npts=[np;npts];qual=[q;qual];mdep=[mdp;mdep];
typ=strvcat(t,typ);src=strvcat(s,src);isunk=[isu;isunk];
save e7700 coords dt time temp mdep npts qual depth nkept bath src typ isunk
!rm e7701.mat
