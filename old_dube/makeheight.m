...
load /Users/johnlyman/data/Globalhc/Levitus/slevhr lon lat dep levsal
...
%  end
  ...
  ii=find(bath<750);jj=find(bath>=750);
  p=sw_pres(depth'*ones(1,length(coords(:,2))),coords(:,2)')';
  height=temp(:,1);
  % do profiles in deep water
  for iz=1:length(jj)
  height(jj(iz))=trapz(squeeze(p(jj(iz),:)),sw_svan(sal(jj(iz),:)',temp(jj(iz),:)',p(jj(iz),:)') )'*1e5;
  end
  % profiles in water < 750m deep
  for l=1:length(ii)
      
    ll=find(depth<=bath(ii(l))&~isnan(temp(ii(l),:)));
    height(ii(l))=trapz(squeeze(p(ii(l),ll)),sw_svan(sal(ii(l),ll)',temp(ii(l),ll)',p(ii(l),ll)'))'*1e5;
  end
  clear temp ii ll sal p coords date bath
  ht=[ht;height];

  disp([num2str([i toc length(height)]),'  ',d(i).name])
  clear height
...
cd '..'
save allheight ht bt cds dt wnum bln blt tm s t
