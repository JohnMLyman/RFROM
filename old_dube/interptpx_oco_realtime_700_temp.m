...
tic,for i=1:length(d)-1
  load(['../Mtpers/realtime_2012/',d(i).name],'sshanom')
  poo=[sshanom(542:end,:);sshanom(1:541,:)];
  s(:,:,1)=[poo(end-1:end,:);poo;poo(1,:)];
  load(['../Mtpers/realtime_2012/',d(i+1).name],'sshanom')
  poo=[sshanom(542:end,:);sshanom(1:541,:)];
...
s=sss;
save allheat_oco_realtime_700_temp dt cds bt ht topex wnum bln blt tm s t
