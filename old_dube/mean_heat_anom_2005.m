...
load landmask msk2
[lon,lat,time,ht]=load_idl_data_mean_heat_anom('../SAL/Floats/heat_no_mean_total.nc');
[lon_mean,lat_mean,time_mean,ht_mean]=load_idl_data_mean_heat('../SAL/Floats/mean_1960_2000.nc');
% repeat 2000 and 1960 so that the linear interpitaion will work
n_mean=length(time_mean);
ht_mean=cat(3,ht_mean(:,:,1),ht_mean, ht_mean(:,:,n_mean));
time_mean=cat(1,time_mean(1)-8,time_mean,time_mean(n_mean)+8);

[nlon,nlat,ntime]=size(ht);

htm=interp3(lat_mean,lon_mean',time_mean,ht_mean,lat,lon',time);

ht=(ht+htm);

clear lat_mean lon_mean time_mean ht_mean
...
hc_total(i)=1e9.*nansum(arw(:).*corrhc(:))/sum(arw(~isnan(msk2(2:end-1,:))));
time_hc(i)=time(i);
