...
function mapdiff_gen_allheat_mon(file_name,fname_nc,clim_file)
...
%remove the annual trend from ht

%  MAKE SURE THAT FILENAME TREND MATCHES FILEMANE AND CLIM_FILE!!!!!! 
%  DO NOT SUPRESS THE OUTPUT OF FILE_NAME_TREND!!!!! 
%  YOU MUST CHEACK IT VERSE THE FILESE THAT YOU INPUTED

'IS THIS RIGHT FILE?'  

file_name_trend='htanom_1993_2007_2004_2007_clim_2'

'IF YOU DO NOT KNOW FIND OUT!!!!'

[hc1,time_one,hc_one,per_cover]=heat_curv_gen_nc_per_cover(file_name_trend);
...
for in=1:nt
    ...
    junk_t=floor(time_one(in));
    year=floor(dt(:,1));
    good=find(year == junk_t);
    
    ht(good)=ht(good)-(hc_one./area_of_ocean)
