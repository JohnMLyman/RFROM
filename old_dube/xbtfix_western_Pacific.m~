...
tic,for i=1:length(tgrid)
  ...
  lat_s_bad=find((lat <= -20)|(lat >= 0));
  lat_n_bad=find((lat <= 0)|(lat >= 20));
  lat_a_bad=find((lat <= -20)|(lat >= 20));
  ...
  hmap_south(:,lat_s_bad)=nan;
  hmap_north(:,lat_n_bad)=nan;
  hmap_all(:,lat_a_bad)=nan;
  ...
  hc_south(i)=nansum(arw(:).*hmap_south(:))/sum(arw(~isnan(hmap_south)));
  hc_north(i)=nansum(arw(:).*hmap_north(:))/sum(arw(~isnan(hmap_north)));
  hc_all(i)=nansum(arw(:).*hmap_all(:))/sum(arw(~isnan(hmap_all)));
...
save hcseries_western_pacific hc_south hc_north hc_all tgrid 
