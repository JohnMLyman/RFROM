...
function mapdiff_gen_twin_mon_all(file_name,fname_nc,min_year,max_year)

for iyear=min_year:max_year
    
    
eval(['load ./twin/',file_name,num2str(iyear) ,' dt ht cds tm topex '])
...
eval(['save ',fname,' yr tpx coords hctpx ht'])
nc_idl_save_hdata_twin_gen_all(['./twin/',fname_nc,num2str(iyear)])
end
