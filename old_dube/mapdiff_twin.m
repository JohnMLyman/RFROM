...
eval(['load allheat_twin_',num2str(iyear) ,' dt ht cds tm topex wnum'])
...
% the slope used here is a 2.4 mm/year slope from Willis 2004
%       here it is expressed as cm/day


 

slope_trend=(0.24)/365.25
htanom=htanom*0.+slope_trend*(day-min(day))
...
gind=find(~isnan(msk));bind=find(isnan(msk));
 save landmask_twin msk lon lat msk2 lat2 lon2
...
save -ascii map_twin.grd outcds

% save grid info to file so we can read output from fortran prog.
save gridinfo_twin mcds gind bind lon lat tgrid
...
for i=1:length(tgrid)
  ii=idx{i};
  fname=['hdata_twin_',num2str(iyear),'_',num2str(fix(tgrid(i)))];
  fname=[fname,'.txt']
