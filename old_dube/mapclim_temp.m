function []=mapclim_temp(depth_level)
...
eval(['load alltemp_',num2str(depth_level)])

load /Users/johnlyman/data/Globalhc/Levitus/tlevhr_700
pos_depth=find(dep == depth_level);

% interpolate levitus heat content onto data
levtemp=[levtemp(end-40:end,:,pos_depth);levtemp(:,:,pos_depth);levtemp(1:41,:,pos_depth)];
lon=[-190.125:.25:190.125];
levtclim=interp2(lon,lat,levtemp',cds(:,1),cds(:,2));
tempanom=temp_lev-levtclim;
...
'cat'
gind=find(day>=datenum(2004,1,1)&day<datenum(2009,1,1)& ...
	~isnan(tpx+tempanom));
tempanom=tempanom(gind);cds=cds(gind,:);day=day(gind);
...
tic,for i=1:length(ggx(:))
  ...
  %end
  %if ~isempty(ii) 
    tmpal(i)=tpx(ii)\tempanom(ii);
    tmpcc(i)=correl(tpx(ii),tempanom(ii));
    tmpxx(i)=sqrt(nanmean(tempanom(ii).^2));
  %else
  %  tmpal(i)=0;
  %  tmpcc(i)=0;
  %  tmpxx(i)=0;
...
alat=lat;alon=ln;
eval(['save /Users/johnlyman/data/Globalhc/HC/tregress_',num2str(depth_level),' alat alon alpha crl xvar'])
%return
cds=cd;
...
clear dt bt gind levtclim temp_lev w alpha bln blt bt d day dd dt mln mlt
...
gtempclim=map+levtemp(ii,:);
gtempclim2=map+levtemp(ii,:)-tpxmap;

eval(['save /Users/johnlyman/data/Globalhc/HC/clim_temp_2004_2008_',num2str(depth_level),' gtempclim gtempclim2 ln lon lat map tpxmap levtemp']) 
