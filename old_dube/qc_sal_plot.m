...
yd=dt(:,1)+(dt(:,2)-1)/12.+(dt(:,3)-1)/365.;
  ...
  temp2=temp;
  sal2=sal;
%caluculate the pontential density
...
dsal=sal(:,1:nd-1)-sal(:,2:nd);
dtemp=temp(:,1:nd-1)-temp(:,2:nd);
ii=[]


for ijunk=1:length(good_times)
   npoint=length(finite(dsal(ijunk,:)));
   nsame=length(finite(unique(dsal(ijunk,:))));
   npoint_temp=length(finite(dtemp(ijunk,:)));
   nsame_temp=length(finite(unique(dtemp(ijunk,:))));
   if ((nsame/npoint) <= .1) | ((nsame_temp/npoint_temp) <=.1)
       ii=[ii,ijunk];
   end
end

%get rid of profiles with a maxium value less than .
...
subplot(2,4,8)
% 
 plot(pden_top,-1*depth(1:nd-1),'.')
 title(['del den ',num2str(round(100*length(jj)./length(good_times))),' %'])
...
% % %     per_bad_den=100*length(jj)./length(good_times);
% % %  eval(['save den_',d(isquare).name,' coords dt time ',...
% % %         'temp qual depth mdep npts id sal fpress per_bad_den'])
% % % end
 end