...
% subsec data to reflect just the c data

offset=fix(1993.5-tgrid(1));

tgrid=(1:length(d)-offset)+1992.5;
d=d(offset+1:end);
...
% make sure all is good
  ...
  W=tmap./sshave;
  error_est=(sshave-tmap).*error;
  error_est_tp=(sshave).*error;
...
% 	'alpha','hmap*','lon','lat','tmap')
  ...
  error_globe(i)=sqrt(nansum((arw(:).*error_est(:)).^2))/(Area_tp*sqrt(N_real/N_tp));
  error_globe_tp(i)=sqrt(nansum((arw(:).*error_est_tp(:)).^2))/(Area_tp*sqrt(N_real/N_tp));
  
  
  error_map(:,:,i)=error_est;
  W_map(:,:,i)=W;
  tp_map(:,:,i)=tp_heat;
  disp(num2str([i toc]))
...
save h_error_est_new tgrid alpha error error_globe error_globe_tp lat lon wnum error_map
