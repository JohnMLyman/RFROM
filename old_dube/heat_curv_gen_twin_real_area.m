function [hctpx,sshtpx]=heat_curv_gen_twin_real_area(tgrid)

current_dir=cd('/home/shoko2/wills/globalhc_dirs/Globalhc/HC');
...
Area=nansum(arw(~isnan(tpxest(:))));

hctpx(i)=nansum(arw(:).*tpxest(:))./Area;


sshtpx(i)=nansum(arw(:).*sshave(:))./Area;
