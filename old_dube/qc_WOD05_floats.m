...
%         end
    %only look at profiles where the mean pressure is less then 1000 dbar
        if (length(find(finite(den_temp) ==1 )) >=2 & nanmean(den_press) > 500)
            ...
            ii_total=[ii,ind_sal',ind_temp',ind_press'];
            %find the bad places using all World Ocan Data base (no floats)
            [ind_temp]=ind_out_qua_wod_floats(den_temp,2);
            [ind_sal]=ind_out_qua_wod_floats(den_sal,2);
            [ind_press]=ind_out_qua_wod_floats(den_press,2);
            ii_wod=[ii,ind_sal',ind_temp',ind_press'];
