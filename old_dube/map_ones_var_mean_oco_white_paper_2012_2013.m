function [corrhc_one_total,lon,lat,time_hc]=map_ones_var_mean_oco_white_paper_2012_2013(start_year,end_year,file_ones_old,file_ones_new)
...
eval(['load ',file_ones_old]);

time_old=time;
ht_old=ht;
one_old=one;
eval(['load ',file_ones_new]);
time=[time_old;time(end-1:end)];
ht=cat(3,ht_old,ht(:,:,end-1:end));
one=cat(3,one_old,one(:,:,end-1:end));
