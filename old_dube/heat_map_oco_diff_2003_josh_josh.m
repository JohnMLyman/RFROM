...
%ncload('htanom_john2_2002_2006.nc','lat','lon','time','one','htdiff');
%htdiff=permute(htdiff,[3 2 1])./1e9;
%one=permute(one,[3 2 1]);
%ind_2006=find(time == 2003.5);

%ht_2006=htdiff(:,:,ind_2006);
%one_2006_josh=one(:,:,ind_2006);
...
ncload('htanom_josh_march2_2003_2003.nc','lat','lon','time','one','ht');
ht=permute(ht,[3 2 1])./1e9;
one=permute(one,[3 2 1]);
ind_2003=find(time == 2003.5);

ht_2003=ht(:,:,ind_2003);
%one_2006_john=one(:,:,ind_2006);

%one_2006=one_2006_john-one_2006_josh;
%mask out bad data
%bad=find(one_2006 <= .8);
%ht_2006(bad)=NaN;
...
ht_2003_john=ht(:,:,ind_2003);
%one_2005=one(:,:,ind_2005);
%one_2005=one_2006_josh;
% mask out bad data
%bad=find(one_2005 <= .8);
%ht_2005(bad)=NaN;
...
corrhc=interp2(lat,lon,ht_2003_john-ht_2003,lat_tpx,lon_tpx');
...
t1=text(40,55,'2003 (josh_{new}-josh_{old})','fontsize',16,'fontweight','bold');
axis([30 390 -90 90])
axis equal
axis([30 390 -90 90])
...
corrhc=interp2(lat,lon,ht_2003,lat_tpx,lon_tpx');
corrhc(isnan(corrhc))=0;
...
t1=text(60,50,'2003 Josh','fontsize',16,'fontweight','bold');
axis([30 390 -90 90])
axis equal
axis([30 390 -90 90])
set(gca,'xtick',[30:30:390],'tickdir','out','xticklabel', [30:30:180,-150:30:30],'ytick',[-90:30:90])
...
%eval(['print -dpng -f1 /home/shoko/C/','''IDL ps''','/heat/oco/2006/oco_john_josh_2003_diff'])
eval(['print -dpng -f1 /home/shoko/C/','''IDL ps''','/heat/oco/2006/oco_john_josh_2003_diff_josh_josh2'])
%eval(['print -dpng -f2 /home/shoko/C/','''IDL ps''','/heat/oco/2006/oco_josh_2003_ht'])
