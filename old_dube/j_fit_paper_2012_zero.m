function [y_model,y_model_err_95,t_out,slope_error,slope]=j_fit_paper_2012_zero(t,yin,scale)
...
% slope_error the one standard error confidence on the slope
...
t_out=[1:100];
y_model=(t_out*model(1)).*scale_fit;
y_model_err_95=(student(dof)*sqrt(((t_out- mean(t_out))*model_err(1)).^2)).*scale_fit;

% % % % 
% % % % y_model=(t_out*model(1)+model(2)).*scale_fit;
% % % % y_model_err_95=(student(dof)*sqrt(model_err(2)+((t_out- mean(t_out))*model_err(1)).^2)).*scale_fit;
...
%slope_error=student(dof)*model_err(1);
