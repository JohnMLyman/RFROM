...
total_years_map=[1993,1994,1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,2005];
total_years=[1950:2005];
%total_years=[1971:2005];
% for iplace=1:12
...
tic,for i=1:length(total_years_map)
  ...
  linear_trend=interp2(glat,gln,map(:,:,2),lat,lon');
  ...
  save(['hc2',d(i).name(4:18)],'corrhc','sshave',...
	'alpha','hmap*','lon','lat','tmap','linear_trend')

  % keep global average time series as we go
  hmap(isnan(corrhc))=NaN;linear_trend(isnan(corrhc))=NaN;
  ...
  lt(i)=nansum(arw(:).*linear_trend(:))/sum(arw(~isnan(linear_trend)));
...
save (['hcseries2_twin_',num2str(iyear)], 'hc', 'df', 'tp', 'hs', 'ds', 'ts', 'tg', 'tgrid', 'lt', 'tsub', 'tpave')
