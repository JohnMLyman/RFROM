...
year_of_oco_pub=2018;
file_name='argo_2018_12_6_QC'
file_name='argo_2018_1_5_QC'
...
file_name_argo='pfloat_sal_greg_jan_2018_QC'


depth_top_plot=0;
depth_bot_plot=700;
slope_min_year=1993;
OHCA_map_name=['hdata_new_layers__ishii_EN3_2014_',file_name,'1950_2018_']
OHCA_map_name=['hdata_new_layers__ishii_EN3_2014_',file_name,'1990_2018_']
%NEED TO CHANGE TO MATCH OUTPUT OF OCO_MAPS_2017_2
...
hc_tpx_eof=nans(nlon_tpx*nlat_tpx,ntime);
ht_all=hc_tpx;
htdiff_all=hc_tpx;
load ../../HC/landmask msk2 lon2 lat2
mask2=interp2(lat2,lon2,msk2,lat_tpx,lon_tpx);
...
for itime=1:ntime
    ...
    ht_junk=reshape(ht(:,:,itime),nlon,nlat);
corrhc=interp2(lat,lon,htdiff_junk,lat_tpx,lon_tpx);
 corrhc(isnan(corrhc))=0;
% corrhc(isnan(mask2))=NaN;
corrhc_dif=corrhc;
corrhc_dif(isnan(mask2))=NaN;
htdiff_all(:,:,itime)=corrhc_dif;

corrhc=corrhc+tpxest(:,:,itime);
corrhc(isnan(mask2))=NaN;
hc_tpx(:,:,itime)=corrhc;
% hc_tpx_eof(:,itime)=corrhc(:);


corrhc=interp2(lat,lon,ht_junk,lat_tpx,lon_tpx);
% corrhc(isnan(corrhc))=0;
corrhc(isnan(mask2))=NaN;
...
% % [lamda,f_x,f_t,error]=j_eof_mis2(hc_tpx_eof');
% % fx=reshape(f_t,[nlon_tpx,nlat_tpx,29]);
% % ft=f_x;

% % eval(['save  tpx_eof_2019  fx hc_tpx hc_tpx_eof time lat lon lamda ft error'])

eval(['save  test_hc  hc_tpx tpxest ht_all htdiff_all time lat lon '])
...
% 
% 
%         eval(['save ',heat_slope_file,'_',num2str(del_year),'_yeartrend_',num2str(start_slope),' slope error time lat lon'])
%        
%     end
% end
cd,original
