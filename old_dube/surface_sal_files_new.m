...
lev_sss=squeeze(sal(:,:,1,:));
lev_lon=[lon(end-40:end)-360; lon; 360+lon(1:41)];
lev_lat=lat;
lev_time=[time(end)-365 time 365+time(1)];
lev_sss=[lev_sss(end-40:end,:,:);lev_sss;lev_sss(1:41,:,:)];
lev_sss=cat(3,lev_sss(:,:,end),lev_sss);
lev_sss=cat(3,lev_sss,lev_sss(:,:,1));



cd('/Volumes/Data/Globalhc/SAL/Floats/argo');
d=sdir(['den_den*.mat']);
...
time_surface(bad)=[];
eval(['save ',filename,'_2011.mat  time_surface coords_surface dt_surface surface_sal_surface']);
