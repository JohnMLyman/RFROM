function [coords_tot,dt_tot,mdep_tot,npts_tot,press_0_tot,sal_0_tot,temp_0_tot, ... 
        ...
        load_data_wmo_square(wmo,del_deg)
...
type_data_tot=[];
time_tot=[];
[wmo_squares,min_lon,max_lon,min_lat,max_lat]=find_wmo_square(wmo,del_deg);
min_lon;
max_lon;
min_lat;
max_lat;
...
for i=1:length(wmo_squares)
    ...
    exist(['w',num2str(wmo),'_1000m.mat'])
    

   if exist(['w',num2str(wmo),'_1000m.mat']),
      eval(['load w',num2str(wmo),'_1000m.mat;']);
    wmo_squares(i)
    lon_junk=coords(:,1);
    lat_junk=coords(:,2);
    b=0;
