...
load clim_no_argo gheatclim2 lon lat ln
load allheat dt ht cds tm topex wnum t
load float_indicies_april18_2006
% get rid of argo baised on calling cards
ind=[];
for i_place=1;4
   junk_ind1=find(s(:,i_place) =='A');
   junk_ind2=find(s(:,i_place) =='a');
   junk_ind3=find(t(:,i_place) =='P');
   junk_ind4=find(t(:,i_place) =='p');
   ind=[ind',junk_ind1',junk_ind2',junk_ind3',junk_ind4']'; 
end
...
save hregress2_no_josh_argo alat alon alpha c xvar
end % of skip
...
save landmask_no_josh_argo msk lon lat msk2 lat2 lon2
...
save -ascii map_no_josh_argo.grd outcds

% save grid info to file so we can read output from fortran prog.
save gridinfo_no_josh_argo mcds gind bind lon lat tgrid
...
for i=1:length(tgrid)
  ii=idx{i};
  fname=['hdata_no_josh_argo_',num2str(fix(tgrid(i)))];
