...
% use topex w/out annual cycle removed
tpx=topex(:,1);
...
for i=1:length(w)
  ...
  [y_model,y_model_err_95,slope_error,slope]=j_fit_alpha_remove_std(tpx(ii)',htanom(ii)');
  ...
  % compute the error by looking at the year to year error
  
  fit_by_yeat_tp_ht_no_plot;
  
  error(i)=slope_error_new;
...
clear i ii tlon tlat hccc hcxx hcal al dt ggx ggy j jj ll poo
 save hregress_seasonal alat alon alpha error error_con alpha_con
end % of skip
