...
eval(['load ',file_path_out,'allheat_300_700_900_1800_monthly_monthly ht_300 ht_700 ht_900 ht_1800 topex cds dt tm mdep npts time fptot fpkeep bdt bbas ind id qual ',...
...
tpx(isnan(tpx))=0;
    ...
    good_time=find( dt(:,1) >= min_year-1800 & dt(:,1)<= max_year+1800);
    ...
    hctpx_300(good_time)=tpx.*interp2(alat,alon,alpha_300,cds(good_time,2),cds(good_time,1));
    hctpx_700(good_time)=tpx.*interp2(alat,alon,alpha_700,cds(good_time,2),cds(good_time,1));
    hctpx_900(good_time)=tpx.*interp2(alat,alon,alpha_900,cds(good_time,2),cds(good_time,1));
    hctpx_1800(good_time)=tpx.*interp2(alat,alon,alpha_1800,cds(good_time,2),cds(good_time,1));
    
    

% 
% 
% [alat,alon,alpha_300,c_300,xvar_300]=compute_alpha(ht_300(:),topex(:,1),dt(:,:),cds(:,:),wnum(:)');
%     [alat,alon,alpha_700,c_700,xvar_700]=compute_alpha(ht_700(:),topex(:,1),dt(:,:),cds(:,:),wnum(:)');
%     [alat,alon,alpha_900,c_900,xvar_900]=compute_alpha(ht_900(:),topex(:,1),dt(:,:),cds(:,:),wnum(:)');
%     [alat,alon,alpha_1800,c_1800,xvar_1800]=compute_alpha(ht_1800(:),topex(:,1),dt(:,:),cds(:,:),wnum(:)');
%      
...
fname_nc=[file_path_hdata,'hdata_monthly_removed_',file_name];
    
    
coords = cds;

eval(['save ',fname_nc,' htdiff_300 htdiff_700 htdiff_900 htdiff_1800 tpx yr coords ht_300 ht_700 ht_900 ht_1800'])
