...
load('/Volumes/Data/Globalhc/HC/hdata_new_table1_EN3_pfloat_sal_greg_jan_2003_2011_100_real_mon.mat')
ht1=ht;
load('/Volumes/Data/Globalhc/HC/hdata_new_table1_EN3_pfloat_sal_greg_jan_2003_2011_100_300_real_mon.mat')
ht2=ht;

load('/Volumes/Data/Globalhc/HC/hdata_new_table1_EN3_pfloat_sal_greg_jan_2003_2011_300_700_real_mon.mat')

ht3=ht;
load('/Volumes/Data/Globalhc/HC/hdata_new_table1_EN3_pfloat_sal_greg_jan_2003_2011_900_real_mon.mat')
ht4=ht;

load('/Volumes/Data/Globalhc/HC/hdata_new_table1_EN3_pfloat_sal_greg_jan_2003_2011_1800_real_mon.mat')

ht5=ht;


%% add in the seasonal cycle

% load in the seasonal cycle
...
eval(['load ',file_path,file_name,'_',idepth','_',num2str(signal_to_noise),'_season_gausian_bin_new_mon_dia_off.mat ht_out lat_grid lon_grid time_grid ']);

eval(['mean_heat_oa_',idepth','=ht_out;']);

 end
% chage the size to intripolate
...
% add the seasonal cycle back in 


year_frac=mod(time,1);
[xi,yi,zi] = meshgrid(lat, lon, mod(time,1));

for jdepth=1:length(depth_name);
    idepth=depth_name{jdepth}';
    
    eval(['ht_out=mean_heat_oa_',idepth',';']);
    seasonal_cycle=interp3(lat_grid,lon_grid,time_grid,mean_heat_oa_100,xi,yi,zi);
    eval(['ht',num2str(jdepth),'_sea=ht',num2str(jdepth),'+seasonal_cycle;'])
...
%%
ht=ht1_sea+ht2_sea+ht3_sea+ht4_sea+ht5_sea;

ind_2008=find(time == 2011+10/12);
...
hold on
m_coast('patch',[1 1 1]);
t1=m_text(60,54,'a) OCT 2011','fontsize',11,'fontweight','bold');
...
corrhc=(corrhc).*1e9./((60*60*24*365.5./2));
corrhc(isnan(msk2(2:end-1,:)))=NaN;
...
hold on
m_coast('patch',[1 1 1]);
t1=m_text(30,54,'b) OCT 2011- March 2011','fontsize',11,'fontweight','bold');
...
caxis([min_val max_val-del_cont])
xlabel('6-month Heat Content Change [W m ^{-2}]')
...
%%% print plot
wysiwyg;orient tall
eval(['print -dpng -f1 /Users/johnlyman/figs/oco/Oceans/oco_heat_oct_2011_mon'])
eval(['print -dpdf -f1 /Users/johnlyman/figs/oco/Oceans/oco_heat_oct_2011_mon'])
