...
w=sdir('./WOD05/trend/w*.mat');
g=sdir('./GTSPP/netcdf/g*.mat');
f=sdir('./Floats/f*.mat');
...
for i=1:length(d)
  ...
  if exist(['./WOD05/trend/w',num2str(d(i)),'.mat']);
    load(['./WOD05/trend/w',num2str(d(i)),'.mat']);
  ...
  % load GTSPP datal
if exist(['./GTSPP/netcdf/g',num2str(d(i)),'.mat']);
    load(['./GTSPP/netcdf/g',num2str(d(i)),'.mat']);
  ...
  % load float data
  if exist(['./Floats/f',num2str(d(i)),'.mat']);
    load(['./Floats/f',num2str(d(i)),'.mat']);
    if size(mdep,1)==1,mdep=mdep';end
 ...
 %%%%%%%%not in all three data bases.
  ...
  % save to new file
  save(['./All_Data/trend/d',num2str(d(i)),'.mat'],'temp','coords','dt','npts','time', ...
