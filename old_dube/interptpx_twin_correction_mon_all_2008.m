...
max_year=2007
max_year_topex=2007;
file_path_out='/Users/johnlyman/data/Globalhc/HC/'
cd /Users/johnlyman/data/Globalhc/HC
for iyear=1993:max_year

%load john_correction_data
load /Users/johnlyman/data/Globalhc/HC/allheat
%ht=htanom;
...
eval(['load ',file_path_out,'aviso_clim_2004_2007 lon lat aviso date_aviso day_aviso']);
...
for i=1:length(day_aviso)-1
    
    
    ii=find(day>=day_aviso(i)&day<=day_aviso(i+1));
    w1=(day_aviso(i+1)-day(ii))/7;w2=(day(ii)-day_aviso(i))/7;
  topex(ii,1)=interp2(lon,lat,aviso(:,:,i)',coords(ii,1),coords(ii,2)).*w1+...
	  interp2(lon,lat,aviso(:,:,i+1)',coords(ii,1),coords(ii,2)).*w2;

end


ff='allheat_twin_correction_mon_all_2008_,',num2str(iyear)
eval(['save ./twin/correction/allheat_twin_correction_mon_all_2008_',num2str(iyear),' dt ht cds topex tm'])
end
