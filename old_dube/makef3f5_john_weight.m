...
%   is computed from a TOPEX estimate of SSH
% 1.1e7 is a possible systematic error from TOPEX that was computed in Wills 2004 
% the student t test was done on the percent and the 1.14 is a correction
%   from wills to accout less varience in the differnece estimate

mean_df=sum(df)/length(df);
samp_error=sqrt((per_var2*(sum((df-mean_df).^2)/length(df))*1.14)/dof);

error=sqrt((student(36)*.6e7)^2+samp_error.^2)+slope_error2*student(dof);
std=(sqrt((sqrt(36)*.6e7)^2+(sqrt(13)*samp_error./student(dof)).^2)+sqrt(13)*slope_error2);

%error=sqrt((student(36)*.6e7)^2+samp_error.^2);
samp_error=error;
close all
figure(1),clf
...
set(gca,'fontsize',16)
e=errorbar(tgrid(old),hc(old),error(old),'k');set(e,'linewidth',1)
...
% [y_model,y_model_err_95,slope_error_tp,sl_tp]=j_fit_weighted(tgrid,tp,w)
 %[y_model,y_model_err_95,slope_error_df,sl_df]=j_fit_weighted(tgrid,df,w)
 [y_model,y_model_err_95,slope_error_hc,sl_hc]=j_fit_weighted(tgrid,hc_2,w_2)

 slope_error_hc=slope_error_hc*1e8;
 sl_hc=sl_hc*1e8;

%[y_model,y_model_err_95,slope_error_tp,sl_tp]=j_fit(tgrid,tp)
%[y_model,y_model_err_95,slope_error_df,sl_df]=j_fit(tgrid,df)
%[y_model,y_model_err_95,slope_error_hc,sl_hc]=j_fit(tgrid,hc_2)
...
% w2_err=num2str(slope_error_df/86400/365.25,'%4.2f');
...
txt={'warming rates:'; ...
['            ',w3,'\pm',w3_err,' W/m^2']};
txth=text(1997.75,-1.9e8,txt,'fontsize',16);
pp4=plot([1996 1997]+1, ...
...
% print plot
print -deps2 -f1 /home/shoko/C/'IDL ps/'heat/f3_john2w.eps
print -dpng -f1 /home/shoko/C/'IDL ps/'heat/f3_john2w.png
...
axis([min_year max_year .0e8 1.5e8])
set(gca,'fontsize',16)
...
set(a,'YAxisLocation','right','color','none','ygrid','off', ...
'xgrid','off','box','off','xtick',[])
 axis(a,[min_year max_year .0e8 1.5e8])
set(a,'fontsize',16)
title('sampling error')
% print plot
print -deps2 -f3 /home/shoko/C/'IDL ps/'heat/ferror_john2w.eps
print -dpng -f3 /home/shoko/C/'IDL ps/'heat/ferror_john2w.png
