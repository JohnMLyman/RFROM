function [hc,time,hc_diff]=heat_curv_gen_mat_diff(file_name)
...
%%%%%%%%%% Compute the the Aviso SSH estimate of OCEAN heatcontent.
...
corrhc_diff=interp2(lat,lon,htdiff(:,:,i),lat_tpx,lon_tpx')./1e9;
corrhc_diff(isnan(msk2(2:end-1,:)))=NaN;
corrhc_diff(isnan(sshmean))=NaN;
corrhc_diff=corrhc_diff+tpxest;
...
hc_diff(i)=1e9.*nansum(arw(:).*corrhc_diff(:))/sum(arw(~isnan(corrhc_diff)));
