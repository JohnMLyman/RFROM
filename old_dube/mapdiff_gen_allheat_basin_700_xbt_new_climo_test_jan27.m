...
function mapdiff_gen_allheat_basin_700_xbt_new_climo_test_jan27(start_year,end_year,fname_nc)
...
clim_file='/Users/johnlyman/data/Globalhc/HC/clim_2_1993_2002_700'
file_name='allheat_700_test_jan21'
data_file='/Volumes/Data/Globalhc/SAL/Floats/htanom_test_jan21_1993_2009'

data_file2='/Volumes/Data/xbt/Levitus/Globalhc/SAL/Floats/htanom_1993_2002_1993_2008_700_levitus'


%compute mean from data 
start_year2=1993;
end_year2=2002;
eval(['load ',data_file2,' time lon lat ht']);

good_time=find(time >= start_year2+.5 & time<= end_year2 +.5);
%%

ht_good=ht(:,:,good_time);

mean_ht2=nanmean(ht_good,3);
lat_ht2=lat;
lon_ht2=lon;
mean_ht2=[mean_ht2(end-40:end-1,:);mean_ht2;mean_ht2(2:41,:)];
lon_ht2=[lon_ht2(end-40:end-1)-360;lon_ht2;lon_ht2(2:41)+360];


%%
%%
clear ht_good time ht lat lon

eval(['load ' clim_file ' gheatclim lon lat ln'])
gheatclim=[gheatclim(end-40:end,:);gheatclim;gheatclim(1:41,:)];
ln=[ln(end-40:end)-360,ln,ln(1:41)+360];
map_clim2=interp2(lat_ht2,lon_ht2,mean_ht2,lat,ln');

clear gheatclim lon lat ln lat_ht2 lon_ht2
%%
...
eval(['load ',data_file,' time lon lat ht']);

good_time=find(time >= start_year+.5 & time<= end_year +.5);
%%

ht_good=ht(:,:,good_time);

mean_ht=nanmean(ht_good,3);
lat_ht=lat;
lon_ht=lon;
mean_ht=[mean_ht(end-40:end-1,:);mean_ht;mean_ht(2:41,:)];
lon_ht=[lon_ht(end-40:end-1)-360;lon_ht;lon_ht(2:41)+360];
...
map_clim=interp2(lat_ht,lon_ht,mean_ht,lat,ln');
%% remove climatology
gheatclim=gheatclim+map_clim+map_clim2;
htclim=interp2(lat,ln,gheatclim,cds(:,2),cds(:,1));
clear gheatclim ii mean_ht map_clim

% remove annual cycle
load /Users/johnlyman/data/Globalhc/Levitus/levcyc_700 levheatcyc lon lat mo
...
save hregress_700_test_jan21 alat alon alpha c xvar
end % of skip
...
htdiff=htdiff_w(pos_atl');
tpx=tpx_w(pos_atl');
yr=yr_w(pos_atl');
coords=coords_w(pos_atl',:);
htanom=htanom_w(pos_atl');

eval(['save ',[fname_nc '_atl'],' htdiff tpx yr coords htanom'])
nc_idl_save_hdata_gen([fname_nc '_atl']);

% save the Pacific

htdiff=htdiff_w(pos_pac');
tpx=tpx_w(pos_pac');
yr=yr_w(pos_pac');
coords=coords_w(pos_pac',:);
htanom=htanom_w(pos_pac');

eval(['save ',[fname_nc '_pac'],' htdiff tpx yr coords htanom'])
nc_idl_save_hdata_gen([fname_nc '_pac']);

% save the Indian

htdiff=htdiff_w(pos_ind');
tpx=tpx_w(pos_ind');
yr=yr_w(pos_ind');
coords=coords_w(pos_ind',:);
htanom=htanom_w(pos_ind');

eval(['save ',[fname_nc '_ind'],' htdiff tpx yr coords htanom'])
nc_idl_save_hdata_gen([fname_nc '_ind']);
