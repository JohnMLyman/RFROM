function [zi,zer,zm]=oaxypp(x,y,z,xi,yi,xcor,ycor,err,npts)
%
% function [zi,zer,zm]=oaxypp(x,y,z,xi,yi,xcor,ycor,err,npts)
%
...
% grid point.  
% the output zi is at locations xi and yi, zer 
% is the associated map error, and zm is the 
% weighted mean field.
%2
...
% Find the o (npts or less) data points 
% closest to each grid point and 
% calculate their cross covariances with this  
% grid point

arg=-(xxi*ones(1,n)-x').^2/xcor2-(yyi*ones(1,1)-y').^2/ycor2;
[ss,tt]=sort(-arg);
jj=tt(1:o);
...
arg=-(xx*ones(1,o)-ones(o,1)*xx').^2/xcor2-(yy*ones(1,o)-ones(o,1)*yy').^2/ycor2;
E=exp(arg)+err*eye(o);
...
% F=[ones(size(xx)),xx,yy];
% ws=diag(c(:));
% mean_coefs=ws*F\ws*zz;
% zz=zz-F*mean_coefs;
...
% zi(i)=(c*(E\zz))+[1,xxi,yyi]*mean_coefs;
