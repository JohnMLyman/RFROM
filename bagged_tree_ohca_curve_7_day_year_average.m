...
for ilayer=startlayer:endlayer
    ...
    tree_file_name=[tree_model_file_name,'_',layer_name];
    load([path_tree,tree_file_name,'_7day.mat'], 'ht_estimate')
    
    ht_estimate=ht_estimate.*arwj;
    
    for itime=1:nyears
        jyear=tgrid(itime);
        ht_out(:,:,itime)=mean(ht_estimate(:,:,time_aviso<jyear+.5&time_aviso>=jyear-.5),3,'omitnan');
    end
...
save test_tree_curve_yearly_7day_0_2000_yearly_new_yearave_half.mat tgrid ht_curve basin_curve layer_curve