...
function getalltp_oco_realtime_oco_2024_orca_new(OcoSetUp)
...
path_in_125=OcoSetUp.path_in_125_ssh;


% remove old near reatime files that might still be in the main directory
dnew_real=dir([path_in_125,'nrt*.nc']);

for ifile_real=1:length(dnew_real)
    delete([dnew_real(ifile_real).folder,'\',dnew_real(ifile_real).name])

end

% move the new near reatime files into the main directory

dnew_real=dir([path_in_125,'SEALEVEL_GLO_PHY_L4_NRT_008_046\*\*\*\nrt*.nc']);

for ifile_real=1:length(dnew_real)
    movefile([dnew_real(ifile_real).folder,'\',dnew_real(ifile_real).name],path_in_125)
...
% remove old delayed files that might be in the junk directory
dnew_dt=dir([path_in_125,'dt*.nc']);

for ifile_real=1:length(dnew_dt)
    delete([dnew_dt(ifile_real).folder,'\',dnew_dt(ifile_real).name])
    
end

% move the new delayed files into the junk directoru
dnew_dt=dir([path_in_125,'SEALEVEL_GLO_PHY_L4_MY_008_047\*\*\*\dt*.nc']);
for ifile_real=1:length(dnew_dt)
    movefile([dnew_dt(ifile_real).folder,'\',dnew_dt(ifile_real).name],path_in_125)
