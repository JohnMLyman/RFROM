function []=filter_s_errdap(TreeSetUp)
...
path_nc_erddap_sal_stable=[path_ERDDAP_sal,'netcdf_smooth\',tree_prefix,'\',subdir,'\'];
...
%            file_name_nc= [path_nc_erddap,'RFROM_OHCA_',num2str(iyear),'_',num2str(imonth),'.nc'];
              ...
              if exist(file_name_nc_sal,'file') 
...
%                    [temp]=load_temp_estimate_nc(file_name_nc_temp);
    ...
    %                for itime=1:1
                  for itime=1:size_sal(4)
                       ...
                       for idepth=1:nlayers-1
                           
                               SA_in=squeeze(double(sal(:,:,idepth,itime)));
                               bad=~isfinite(SA_in);
                               SA_in=nanmedfilt2(SA_in,3);
                               SA_in(bad)=nan;
                               sal_out(:,:,idepth,itime)=SA_in;
