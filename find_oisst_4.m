function [sst]=find_oisst_4(lonp,latp,yr)
...
lat_sst=double(ncread([path_oisst,'sst.day.anom.1990.nc'],'lat'));
lon_sst=double(ncread([path_oisst,'sst.day.anom.1990.nc'],'lon'));
...
time=double(ncread([path_oisst,'sst.day.anom.',year_name_min,'.nc'],'time'));
sst_anom_year=ncread([path_oisst,'sst.day.anom.',year_name_min,'.nc'],'anom');
...
for yr_value=load_years
  ...
  if exist([path_oisst,'sst.day.anom.',yr_name_next,'.nc'],'file')
      ...
      time_next=ncread([path_oisst,'sst.day.anom.',yr_name_next,'.nc'],'time');
