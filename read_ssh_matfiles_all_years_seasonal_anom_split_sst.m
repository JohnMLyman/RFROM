function []=read_ssh_matfiles_all_years_seasonal_anom_split_sst(TreeSetUp)
load_TreeSetUp
...
for isplit=1:2
        ...
        [w_art,w_atl]=find_atlantic_artic_overlap_weights(global_basins_aviso,LON,LAT);
         [w_3,w_11]=artic_overlap(global_basins_aviso,LON);
...
%                             jssh=ssh_total(pos_3d);
                            ...
                            jlat=repmat(LAT(pos_2d),nfiles,1);
                             jssh=repmat(ssh_missing(pos_2d),nfiles,1);
                            % compute weights so there is a linear transition at the overlaps of
                            % the Atlantic, Pacific and Indian Oceans.
                        
                            if ibasin==3
                                weight_cross=w_art.*w_3;
                            elseif ibasin==5
                                 weight_cross=w_atl;
                            elseif ibasin==11
                                weight_cross=w_art.*w_11;
                            else
