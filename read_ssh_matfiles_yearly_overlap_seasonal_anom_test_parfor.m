...
for ilayer=2:nlayer
    ...
    ht_estimate=nan(nlon_tpx,nlat_tpx,ntotal_files);
    
    for iyear_mod_start=min_tpx_year+.5:.5*nworkers:max_tpx_year+.5
             
        iyear_mod_end=iyear_mod_start+.5*(nworkers-1);

        if iyear_mod_end > max_tpx_year +.5
            iyear_mod_end=max_tpx_year +.5;

        end
        iyear_mod_vec=iyear_mod_start:.5:iyear_mod_end;
        nvec=length(iyear_mod_vec);
    ...
    %         global_basins_aviso(ibasin).name
        ...
        %% fill cell arrays
            for iyear_mod_pos=1:nvec
                iyear_mod=iyear_mod_vec(iyear_mod_pos);
            ...
            %      ht_estimate(good) = predict(M,input_table);
                 if ~isempty(M)
                     ...
                     parfor iyear_mod_pos=1:nvec
   mat                     iyear_mod=iyear_mod_vec(iyear_mod_pos);
                        year_file_name=num2str(10*iyear_mod)
                        file_big_model=[path_new_tree,tree_model_file_name,'_model_',layer_name,'_',year_file_name,'.mat'];
                        load(file_big_model,'model_all')     
                        n_mod_basin=length(model_all);
