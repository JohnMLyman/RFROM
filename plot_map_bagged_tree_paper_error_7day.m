...
[~,good_s]=min(abs(time_aviso-syear));
good_s=min(good_s);
map_s=ht_estimate(:,:,good_s)-mean_ht;
iframe=good_s;
...
%check this
date_name_junk=datestr(days_since_1950+datenum(1950,1,1),'mmm-dd-yyyy');
[junk_year,junk_month,junk_day]=datevec(days_since_1950+datenum(1950,1,1));
...
%     save 'OHCA_2019_tpx.mat' lon lat corrhc
    ...
    hold on
    m_coast('patch',[1 1 1]);
    t1=m_text(30,100,['(a) ', date_name_junk(iframe,:)],'fontsize',12);
    ...
    load([path_tree,'error_map_0_700_combined.mat'], 'time_aviso', ...
    'ht_error_0_700','lon_tpx','lat_tpx')
    [~,good_s]=min(abs(time_aviso-syear));
     good_s=min(good_s);
    s_error_ht_0_700=ht_error_0_700(:,:,good_s);
    clear ht_error_0_700

    corrhc=(s_error_ht_0_700)./((1e9));
    %%
    lon=lon_tpx';
    lat=lat_tpx';
...
%     save 'OHCA_2019_tpx.mat' lon lat corrhc
    ...
    hold on
    m_coast('patch',[1 1 1]);
    t1=m_text(30,100,['(b) ', date_name_junk(iframe,:),' error'],'fontsize',12);
    %t3=m_text(50,45,[num2str(depth_top_plot),'-',num2str(depth_bot_plot)],'fontsize',12);
     t2=m_text(320,-105,'(10^{9} J m ^{-2})','fontsize',12);
...
%     set(jc,'XAxisLocation','bottom','pos',jcpos-[-.075 .805 .15 0])
    ...
    eval(['print -dpng -r600 -f1 ',path_figs,'oco_heat_tree_error_new_publish_',num2str(year_of_oco_pub),'_for_7day_temp',num2str(floor(fyear))])
