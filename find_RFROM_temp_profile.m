function [lon_out,lat_out,time_out,pres,temp]=find_RFROM_temp_profile(lon_data,lat_data,time_data)
...
files=dir([path_RFROM,'\RFROM_TEMP_*.nc']);
Nfile=length(files);
...
for iprof=1:nprof
        ...
        [iyear,imonth,~]=datevec(double(time_out(iprof))+day_1950);
         ...
         if imonth>=10
              jfile_name_nc= [path_RFROM,'\RFROM_TEMP_',num2str(iyear),'_',num2str(imonth),'.nc'];
           else
              jfile_name_nc= [path_RFROM,'\RFROM_TEMP_',num2str(iyear),'_0',num2str(imonth),'.nc'];
         end
       ...
       temp(iprof,:)=load_temp(jfile_name_nc,ilon,ilat,time_out(iprof));
...
function [temp]=load_temp(file_name,ilon,ilat,jtime)


    temp_total=ncread(file_name,'ocean_temperature');
    time=ncread(file_name,'time');
    pos_time=find(time==jtime);
    temp=squeeze(temp_total(ilon,ilat,:,pos_time));
