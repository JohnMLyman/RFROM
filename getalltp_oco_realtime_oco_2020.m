...
tmax=datenum(2017,6,8)-datenum(1950,1,1);
...
[sla]=read_netcdf_getall_getprofiles(d(jj(1)).name,'sla');
[lat]=read_netcdf_getall_getprofiles(d(jj(1)).name,'latitude');
[lon]=read_netcdf_getall_getprofiles(d(jj(1)).name,'longitude');
...
% %   [sla]=read_netcdf_getall_getprofiles(d(jj(i)).name,'sla');
    [sla]=read_netcdf_sla(d(jj(i)).name);
...
% %      [sla]=read_netcdf_getall_getprofiles([prefex_realtime_dir,djunk(1).name],'sla');
          [sla]=read_netcdf_sla([prefex_realtime_dir,djunk(1).name]);
...
% %     [sla]=read_netcdf_getall_getprofiles([prefex_delayed_dir,djunk(1).name],'sla');
    [sla]=read_netcdf_sla([prefex_delayed_dir,djunk(1).name]);
...
% % %              [sla]=read_netcdf_getall_getprofiles(['realtime_oco/',d(i).name],'sla');
             [sla]=read_netcdf_sla(['realtime_oco/',d(i).name]);
            sla=sla';
             %nc_getall([prefex_realtime_dir,djunk(1).name])
             gmo;
        else
...
% %         [sla]=read_netcdf_getall_getprofiles([prefex_delayed_dir,djunk(1).name],'sla');
    ...
    if d(i).name(1)=='d'
        save(['realtime_oco/ssh',num2str(dy(i))],'sshanom','lat','lon');
    else
        save(['realtime_oco/ssh',num2str(dy(i))],'sshanom','lat','lon','offset');
    end
  disp([d(i).name,'  ',num2str(toc)])
  
    ['realtime_oco/ssh',num2str(dy(i))]
...
for j=1:length(jj)
    ...
    if exist(['./realtime_oco/ssh',num2str(dy(i)),'.mat'],'file')
      
      load(['./realtime_oco/ssh',num2str(dy(i))],'sshanom','lat','lon');
