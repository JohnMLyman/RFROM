...
for ibasin=nbasins_use
       ...
       if ~isempty(ht_mat)
           input_mat=nans(length(ht_mat),6+nextra_layers);
           input_mat(:,1)=yr(good);
           ...
           input_mat(:,6)=temp_use(good);
            % input the shallow layer temp 
            ilayer_extra_tot=6;
            for ilayer_extra=1:top_level_use
                ilayer_extra_tot=ilayer_extra_tot+1;
                eval(['input_mat(:,ilayer_extra_tot)=temp_use_shallow_',num2str(ilayer_extra),'(good);'])
            end
    
            % input the deep layer temp 
            for ilayer_extra=1:bot_level_use
                 ilayer_extra_tot=ilayer_extra_tot+1;
                eval(['input_mat(:,ilayer_extra_tot)=temp_use_deep_',num2str(ilayer_extra),'(good);'])
            end
