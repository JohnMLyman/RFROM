    ...
    %only look at temp contents with more than 2 good points
      ...
      for ilayer=2:length(layer_bounds)
          ...
          % Set temp_junk_name and bad_temp_junk to the layer of interest
             eval(['temp_junk_name=',...
                 'temp_',num2str(layer_bounds(ilayer-1)),'_',num2str(layer_bounds(ilayer)),'_near;'])
             eval(['bad_temp_junk=',...
                 'bad_temp_',num2str(layer_bounds(ilayer-1)),'_',num2str(layer_bounds(ilayer)),';'])
           ...
           % determine if this cast is bad
            if   ~isempty(this_cast) && ~isempty(ind_junk)
                ...
                if ~isempty(find(ind_junk == this_cast, 1)) 
                    bad_temp_junk=[bad_temp_junk',i]';
                    per_done=100.*i/nd
                    eval(['per_bad_temp_',num2str(layer_bounds(ilayer-1)),'_',num2str(layer_bounds(ilayer)),...
                    '=100.*length(bad_temp_junk)/i'])
