...
for ilayer=2:length(layer_bounds)
    ...
    ht_junk=[heat_wod_junk(early_en4);ht_junk];
...
qual_wod=qual_wod(early_en4);
cds=[coords_wod(early_en4,:);cds];
dt=[dt_wod(early_en4,:);dt];
...
mdep=[mdep_wod(early_en4);mdep];
%time=[time_wod;time];

wod_oclnum=[oclnum_wod(early_en4);nans(size(id))];

  



file_name=[file_name,'_argo_only_2005'];
