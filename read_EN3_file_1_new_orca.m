function [coords,dt,time,nprof,qual,fflag,ptype,pinf,ocl,mdep,wnum,data]=read_EN3_file_1_new_orca(ifile,path)
...
%  (p2~='R') and not in WOD05 and marked as a profiling pfloat inst_type~= 9
        ...
        good=find( temp_qc_junk == '1' & pos_qc_junk =='1' &  ...
          ...
          ind=[1:ngood];
           ...
           mdep(ind)=max(depth_junk);
            ...
            for iprof=1:ngood
              ...
              bad= temp_junk(:,iprof) == 99999 |temp_qc2_junk(:,iprof) =='4'|depth_junk(:,iprof)==99999;
              ...
              data{ind(iprof),2}=temp_junk(~bad,iprof)*nan;
