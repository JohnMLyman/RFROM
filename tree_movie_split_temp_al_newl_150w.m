function   tree_movie_split_temp_al_newl_150w(TreeSetUp)
%%
...
nlayer=length(layer_bounds);
 ...
 file_sum_name=[path_tree,tree_model_file_name,'_150w_all.mat'];
...
[~,pos_lon]=min(abs(lon_tpx-210));
[~,pos_lon_topo]=min(abs(lon_topo-210));
...
%%
 ...
 % put into the proper coordinates
    ...
    min_val=-2;
    max_val=28.;
v=VideoWriter([path_figs,'tree_150w_top.avi']);
v.FrameRate = 10;
open(v)
...
%     corrhc=ht_estimate(:,:,iframe)./1e9;
   ...
   corrhc=ht_estimate_mean(:,:,iframe);
    ...
    a=gca;
    set(a,'xtick',[],'ytick',[])
        set(a,'tickdir','out','xtick',[-60:5:65],'ytick',[-500:20:0],...
            'XAxisLocation','bottom');
    yticklabels(([500:-20:0]))
    axis(a,[-62 70 -500 0])
    apos=get(a,'pos');
   title(['150 W on ',num2str(time_aviso(iframe))])
