...
pos_2d_pac_atl=global_basins_aviso(5).pos & global_basins_aviso(2).pos;
   ...
   for itime=2:ntime
      ...
      good_time=yr_all>=time_aviso(itime)-.5 & yr_all<=time_aviso(itime)+.5;
      good_time_aviso=time_aviso>=time_aviso(itime)-.5 & time_aviso<=time_aviso(itime)+.5;
...
%     sst=hold_out_mat(:,5);
    ...
    % the hold out estimates
     ...
     var_ht=var(ht_estimate(:,:,good_time_aviso),1,3,'omitnan');
...
%% sumup over basin
    ...
    %         global_basins_aviso(ibasin).name
            ...
            scale_var(pos_2d)=nansum([scale.*weight_cross(pos_2d) scale_var(pos_2d)],2);
 ...
 save([path_tree,tree_file_name_yearly_error,'_7day.mat'], 'ht_error', 'lon_tpx', 'lat_tpx', 'time_aviso',...
    '-v7.3')
