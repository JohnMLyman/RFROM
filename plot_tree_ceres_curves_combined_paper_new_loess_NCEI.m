...
var_toa_all=var(toa);
var_toa_all_qu=var(toa_qu+cycle_toa);
...
var_gilson_all_qu=var(gilson_rate_qu);
...
frac_gilson_all_qu=var_gilson_all_qu./var_toa_all_qu;
...
plot(time_gilson_rate_qu,gilson_rate_qu,'color',mycor(4,:),'LineWidth',1.25)
text(2009,-30,[name_comp,' ratio= ',num2str(round(frac_gilson_all_qu))],'color',mycor(4,:),'fontsize',8)
...
d = Document([path_figs,'Tree_corr_table_loess_new'],'docx');
append(d,mltableObj);
close(d);
% rptview(d);
...
%% table for RG09 
% max_cor_gil=max_cor_gil_qu;
max_cor_gil_mon=max_cor_gil_qu;
% cor_0_gil=cor_0_gil_qu;
cor_0_gil_mon=cor_0_gil_qu;
% max_lag_gil=max_lag_gil_qu;
max_lag_gil_mon=max_lag_gil_qu;
frac_gilson_all=frac_gilson_all_qu;
frac_gilson_mon=frac_gilson_qu;
...
cor_0_gil=num2str([max_lag_gil;max_lag_gil_year;max_lag_gil_qu;max_lag_gil_mon],3);
...
import mlreportgen.dom.*
mltableObj = MATLABTable(mltable);
d = Document([path_figs,name_comp,'_corr_table_loess_new'],'docx');
append(d,mltableObj);
close(d);
% rptview(d);
