function tree_movie_temp_100_GulfStream(TreeSetUp)
...
movie_name=['temp_100m_',num2str(floor(end_year)),'_Gulf_2015_newg.gif'];
filename=[path_Figs,movie_name];
% 
% movie_name=['temp_100m_',num2str(floor(end_year)),'_Gulf_2015_new.mp4'];
% 
...
for ifile=1:nfiles
    ...
    for itime=1:ntime
        ...
        clf
            set(gcf,'color','white');
            m_proj('Equidistant cylindrical','long',[280 325],'lat',[32 48]);
...
%             set(h1,'linecolor','none')
            hold on
            m_grid('tickdir','out','xtick',[280:5:325],'ytick',[32:2:48],'linestyle','none');
            ...
            hold on
            m_coast('patch',[0 0 0]);
            t1=m_text(280,49,[' ', date_name],'fontsize',12);
            %t3=m_text(50,45,[num2str(depth_top_plot),'-',num2str(depth_bot_plot)],'fontsize',12);
             t2=m_text(300,25,'         (C^{\circ})','fontsize',10);
