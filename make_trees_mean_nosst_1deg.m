function [model_all]=make_trees_mean_nosst_1deg(use,ht_use,tpx,coords,yr,nbasins_use,good_yr,good_prof,large_scale,scale_box_deg)
...
for ibasin=nbasins_use
       ...
       if ~isempty(ht_mat)
           ...
           if ibasin==2
               ...
               switch large_scale
                   case 'a'
                       input_mat(:,2)=scale_box_deg.*floor(jj_lon./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*floor(coords(good,2)./scale_box_deg);
                   case 'b'
                       input_mat(:,2)=scale_box_deg.*floor(jj_lon./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*round(coords(good,2)./scale_box_deg);
                   case 'c'
                       input_mat(:,2)=scale_box_deg.*round(jj_lon./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*floor(coords(good,2)./scale_box_deg);
                   case 'd'
                      input_mat(:,2)=scale_box_deg.*round(jj_lon./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*round(coords(good,2)./scale_box_deg);
                ...
                switch large_scale
                   case 'a'
                       input_mat(:,2)=scale_box_deg.*floor(coords(good,1)./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*floor(coords(good,2)./scale_box_deg);
                   case 'b'
                       input_mat(:,2)=scale_box_deg.*floor(coords(good,1)./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*round(coords(good,2)./scale_box_deg);
                   case 'c'
                       input_mat(:,2)=scale_box_deg.*round(coords(good,1)./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*floor(coords(good,2)./scale_box_deg);
                   case 'd'
                      input_mat(:,2)=scale_box_deg.*round(coords(good,1)./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*round(coords(good,2)./scale_box_deg);
