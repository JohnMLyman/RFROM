...
mean_depth_bnds=[layer_bounds(1:end-1); layer_bounds(2:end)];
   ...
   for iyear=year_start_nc:year_end_nc
       for imonth=1:12
           ...
           file_name_nc= [path_nc,'RFROM_OHCA_ERROR_',num2str(iyear),'_',num2str(imonth),'.nc'];

           if exist(file_name_mat_nc,'file')
               load(file_name_mat_nc,'ht_error_mon','time_1950','lon_tpx','lat_tpx')
               ntime=length(time_1950);
               nlon=length(lon_tpx);
               nlat=length(lat_tpx);
               ndepth=length(mean_depth);
               ht_error=nan(nlon,nlat,ndepth,ntime);
               ht_error(:,:,1,:)=ht_error_mon;
               ...
               for ilayer=3:nlayers
                   ...
                   if imonth>=10
                      ...
                      file_name_nc= [path_nc,'RFROM_OHCA_ERROR_',num2str(iyear),'_0',num2str(imonth),'.nc'];
                   end
                   ...
                   if exist(file_name_mat_nc,'file')
                       ...
                       load(file_name_mat_nc,'ht_error_mon')
                        ht_error(:,:,ilayer-1,:)=ht_error_mon;
               ...
               write_netcfd_cf_heat_depth_mon_single_error(ht_error,time_1950,lon_tpx,...
