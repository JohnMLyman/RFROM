...
path_fileG= 'C:\Users\jlyma\OneDrive - University of Hawaii\data\Roemmich_Gilson_Clim\';
path_figs='C:\data\OHCA\figs\tree_paper\';
 path_file='C:\Users\jlyma\OneDrive - University of Hawaii\data\OHCA\NCEI\';
...
load([path_file,'NCEI_OHCA_0_2000.mat'],'ohca','yr')
...
ht_gilson_res_qu=ht_gilson_res';
ht_gilson_res_year=smooth(ht_gilson_res,8,'loess');
...
gilson_rate_qu_no_cycle=(ht_gilson_qu_no_cycle(start_ind_qu:end)-ht_gilson_qu_no_cycle(1:end-end_ind_off_qu)).*fac;
gilson_rate_qu=(ht_gilson_qu(start_ind_qu:end)-ht_gilson_qu(1:end-end_ind_off_qu)).*fac;

gilson_rate_year_no_cycle=(ht_gilson_year_no_cycle(start_ind_year:end)-ht_gilson_year_no_cycle(1:end-end_ind_off_year)).*fac./4;
...
% NCEI is only at quatrly resolution

time_gilson_rate=time_gilson_rate_qu;
gilson_rate=gilson_rate_qu;
gilson_rate_no_cycle=gilson_rate_qu_no_cycle;
...
% cycle_toa=model_toa'-mean_toa-slope_toa.*center_year_toa-(toa_time-center_year_all).*slope_toa';
%  the definition of cycle_toa below and above are the same,  the bottom
%  verion is just clearer
...
var_gilson_qu_all=var(gilson_rate_qu);
...
frac_gilson_qu_all=var_gilson_qu_all./var_toa_qu_all;
frac_tree_qu_all=var_tree_qu_all./var_toa_qu_all;
...
save([path_figs,name_comp,'_rate.mat'],...
...
text(2009,-30,[name_comp,' ratio= ',num2str(round(frac_gilson_all))],'color',mycor(4,:),'fontsize',8)
...
%   text_del=text_del*10;
  ...
  text_size=12;
  text_off=-50;
text(year_text,text_off+text_del*4,name_comp,'color',mycor(4,:),'FontName','Arial','FontSize',text_size)
...
set(gca,'tickdir','out','Fontname','Arial','box','on')
text(2009,-.5,[name_comp,' ratio= ',num2str(round(frac_gilson_year))],'color',mycor(4,:),'fontsize',8)
...
% corr(toa_gilson(plot_pos_gilson_year),gilson_rate_year_no_cycle(plot_pos_gilson_year))
 ...
 eval(['print -dpng -r600 -f1 ',path_figs,'toa_tree_',name_comp,'_ceres2_loess'])
%% yearly
...
toa_gilson_qu_all=interp1(toa_time,toa_qu_all,time_gilson_rate_qu);
toa_tree_qu_all=interp1(toa_time,toa_qu_all,time_rate_tree_all_qu);

toa_corr_qu_tree=toa_tree_qu(plot_pos_tree_qu&isfinite(toa_tree_qu))';
toa_corr_qu_tree_all=toa_tree_qu_all(plot_pos_tree_qu&isfinite(toa_tree_qu_all))';


tree_corr_qu=rate_qu_all_no_cycle(plot_pos_tree_qu&isfinite(toa_tree_qu))';
tree_corr_qu_all=rate_qu_all(plot_pos_tree_qu&isfinite(toa_tree_qu))';

toa_corr_qu_gilson=toa_gilson_qu(plot_pos_gilson_qu);
toa_corr_qu_gilson_all=toa_gilson_qu_all(plot_pos_gilson_qu);

gilson_corr_qu=gilson_rate_qu_no_cycle(plot_pos_gilson_qu);
gilson_corr_qu_all=gilson_rate_qu(plot_pos_gilson_qu);
...
[cor_tree_qu_all,lag_tree_qu_all]=crosscorr(toa_corr_qu_tree_all,tree_corr_qu_all);

[cor_gil_qu,lag_gil_qu]=crosscorr(toa_corr_qu_gilson,gilson_corr_qu);
[cor_gil_qu_all,lag_gil_qu_all]=crosscorr(toa_corr_qu_gilson_all,gilson_corr_qu_all);
...
[~,pos_m]=max((cor_tree_qu));
max_cor_tree_qu=cor_tree_qu(pos_m);
max_lag_tree_qu=lag_tree_qu(pos_m);
cor_0_tree_qu=cor_tree_qu(lag_tree_qu==0);

[~,pos_m]=max((cor_tree_qu_all));
max_cor_tree_qu_all=cor_tree_qu_all(pos_m);
max_lag_tree_qu_all=lag_tree_qu_all(pos_m);
cor_0_tree_qu_all=cor_tree_qu_all(lag_tree_qu_all==0);

[~,pos_m]=max((cor_gil_qu));
max_cor_gil_qu=cor_gil_qu(pos_m);
max_lag_gil_qu=lag_gil_qu(pos_m);
cor_0_gil_qu=cor_gil_qu(lag_gil_qu==0);

[~,pos_m]=max((cor_gil_qu_all));
max_cor_gil_qu_all=cor_gil_qu_all(pos_m);
max_lag_gil_qu_all=lag_gil_qu_all(pos_m);
cor_0_gil_qu_all=cor_gil_qu_all(lag_gil_qu_all==0);
...
%   text_del=text_del*10;
  ...
  text_size=12;
  text_off=-15;
text(year_text-.95,text_off+text_del*4,name_comp,'color',mycor(4,:),'FontName','Arial','FontSize',text_size)
...
path_figs='C:\data\OHCA\figs\tree_paper\'
 eval(['print -dpng -r600 -f2 ',path_figs,'toa_tree_',name_comp,'_ceres_cycle_new_loess'])
% 
...
% % % %%  make table
% % % 
% % % Corr_max =num2str([max_cor_tree;max_cor_tree_qu_all;max_cor_tree_year;max_cor_tree_qu;max_cor_tree_mon],3);
% % % Corr_0=num2str([cor_0_tree;cor_0_tree_qu_all;cor_0_tree_year;cor_0_tree_qu;cor_0_tree_mon],3);
% % % Lag_max=num2str([max_lag_tree;max_lag_tree_qu_all;max_lag_tree_year;max_lag_tree_qu;max_lag_tree_mon],3);
% % % Ratio_CERES_Var=num2str([frac_tree_all;frac_tree_qu_all;frac_tree_year;frac_tree_qu;frac_tree_mon],3);
% % % sm_scale={'Month w/annual';'Quart w/annual';'Year';'Quart';'Month'};
% % % mltable=table(Ratio_CERES_Var,Corr_0,Corr_max,Lag_max,'RowNames',sm_scale);
% % % 
% % % import mlreportgen.dom.*
% % % mltableObj = MATLABTable(mltable);
% % % d = Document([path_figs,'Tree_corr_table_loess_fixed'],'docx');
% % % append(d,mltableObj);
% % % close(d);
% rptview(d);
...
%% table for RG09 
Corr_max =num2str([max_cor_gil;max_cor_gil_qu_all;max_cor_gil_year;max_cor_gil_qu;max_cor_gil_mon],3);
Corr_0=num2str([cor_0_gil;cor_0_gil_qu_all;cor_0_gil_year;cor_0_gil_qu;cor_0_gil_mon],3);
Lag_max=num2str([max_lag_gil;max_lag_gil_qu_all;max_lag_gil_year;max_lag_gil_qu;max_lag_gil_mon],3);
Ratio_CERES_Var=num2str([frac_gilson_all;frac_gilson_qu_all;frac_gilson_year;frac_gilson_qu;frac_gilson_mon],3);
sm_scale={'Month w/annual';'Quart w/annual';'Year';'Quart';'Month'};
mltable=table(Ratio_CERES_Var,Corr_0,Corr_max,Lag_max,'RowNames',sm_scale);

import mlreportgen.dom.*
mltableObj = MATLABTable(mltable);
d = Document([path_figs,name_comp,'_corr_table_loess_fixed'],'docx');
append(d,mltableObj);
close(d);
% rptview(d);
