function read_ssh_matfiles_yearly_overlap_seasonal_orca_sal_mean_nosst(TreeSetUp)
...
%%
tic
delta_year_mean_maps=end_year_mean-start_year_mean;
mid_mean_maps=start_year_mean+delta_year_mean_maps./2;
...
for isplit=1:2
        ...
        for iyear=1:nyears 
         ...
         % sometimes there are missing values if there is not a whole year
         % of data this removes thoes missing values
         pos_finite=isfinite(time_aviso);
         ssh_total=ssh_total(:,:,pos_finite);
         sst_total=sst_total(:,:,pos_finite);
         time_aviso=time_aviso(pos_finite);
         nfiles=length(time_aviso);

        if isplit==1
            ssh_total1=single(ssh_total);
            sst_total1=single(sst_total);
            time_aviso1=time_aviso(pos_finite);
...
%             total_junk=repmat(2:nlayer,1,2);
%             ilayer=total_junk(tlayer);
%             isplit=2;
%             if tlayer<=nlayer-1; isplit=1;end
            if isplit==1
...
%              [temp_use,~]=load_monthly_mean_temp(layer_bounds,path_mat_nc,tree_model_file_name_season_temp,ilayer,time_aviso);
               [temp_use,~]=load_monthly_all_temp(layer_bounds,path_mat_nc,tree_model_file_name_combined_temp,ilayer,time_aviso);
        %     start_year=2004;
        %     end_year=2021;
...
%                                 good=isfinite(jyr) & isfinite(jtemp_use)&good_yr;
...
%             clear model_all
            ...
            if isplit==1
                ...
                parsave_tree ([path_tree_junk,tree_file_name_season,'_split_7day_junk1.mat'] ,ht_estimate,lon_tpx, lat_tpx,time_aviso)
            else
...
for ilayer=2:nlayer
    ...
    parsave_tree([path_new_tree_season,tree_file_name_season,'_split_7day.mat'],ht_estimate,lon_tpx, lat_tpx,time_aviso);
