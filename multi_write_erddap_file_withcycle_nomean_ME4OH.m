function []=multi_write_erddap_file_withcycle_nomean_ME4OH(TreeSetUp)
...
if var_type=='h'
    ...
    mean_depth=(layer_bounds_ME4OH_out(1:end-1)+layer_bounds_ME4OH_out(2:end))./2;
    mean_depth=mean_depth';
    mean_depth_bnds=[layer_bounds_ME4OH_out(1:end-1); layer_bounds_ME4OH_out(2:end)];
else
...
[ht_mean]=bagged_tree_ohca_curve_7_day_errdp_ohca_mean_remove_ME4OH(TreeSetUp);
...
for year_load=time_ssh_load
    ...
    days_since_1900=sday-datenum(1900,1,1);
    [~,month_data]=datevec(days_since_1900+datenum(1900,1,1));
    
    for imonth=1:12
        good_month=month_data==imonth;
        time_1900=days_since_1900(good_month);

        if ~isempty(time_1900)
            ...
            elseif var_type=='h'
                write_netcfd_cf_heat_depth_mon_single_nomean_ME4OH(mon_estimate,time_1900,lon_tpx,...
