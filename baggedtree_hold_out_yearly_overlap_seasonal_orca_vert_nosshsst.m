function []=baggedtree_hold_out_yearly_overlap_seasonal_orca_vert_nosshsst(TreeSetUp)
...
elseif ilayer<=ilayer_depth_use_sst
    ...
    [model_all]=make_trees_mean_vert(use,ht_use,tpx,sst,ht_predict,coords,yr,nbasins_use,good_yr,good_prof);

elseif ilayer<=ilayer_depth_use_ssh
    ht_predict=ht_all_junk{ilayer-2}; 
    good_prof=isfinite(ht_use)&isfinite(ht_predict);
    [model_all]=make_trees_mean_vert_nosst(use,ht_use,tpx,ht_predict,coords,yr,nbasins_use,good_yr,good_prof);
else
     ht_predict=ht_all_junk{ilayer-2}; 
    good_prof=isfinite(ht_use)&isfinite(ht_predict);
    [model_all]=make_trees_mean_vert_nosst_nossh(use,ht_use,ht_predict,coords,yr,nbasins_use,good_yr,good_prof);
end



file_big_model=[path_tree_junk,tree_model_file_name_season,'_model_',layer_name,'_',year_file_name,'_split.mat'];
