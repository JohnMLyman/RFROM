function [ht_mean]=bagged_tree_ohca_curve_7_day_errdp_ohca_mean_remove_ME4OH(TreeSetUp)
...
time_load=[start_year_mean_remove:end_year_mean_remove];
...
ht_estimate_junk=double(ncread(file_name_nc,'ocean_heat_content_anomaly'));
s_estimate=size(ht_estimate_junk);
% arw=areavec(lon_tpx,lat_tpx);



n_out=zeros(s_estimate(1),s_estimate(2),s_estimate(3));
ht_out=n_out;
tic
...
%      display(year_load)
     for imonth=1:12
        ...
        if exist(file_name_nc,'file')
            ht_estimate=double(ncread(file_name_nc,'ocean_heat_content_anomaly'));
           
            
               ht_out_junk=ht_estimate;
          
               ht_out_total=squeeze(sum(ht_out_junk,4,"omitnan"));
               n_good=squeeze(sum(double(isfinite(ht_out_junk)),4));
            ...
            ht_out=ht_out+ht_out_total;
        end
     end


end
ht_mean=ht_out./n_out;




mean_name=['mean_',file_type,'_map_',tree_prefix,'_',num2str(start_year_mean_remove),'_',...
    num2str(end_year_mean_remove),'.mat'];

save ([path_tree,mean_name], 'ht_mean','lat_tpx','lon_tpx','-v7.3');
