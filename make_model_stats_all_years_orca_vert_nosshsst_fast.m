function []=make_model_stats_all_years_orca_vert_nosshsst_fast(TreeSetUp)
...
%         file_big_model=[path_tree_junk,tree_model_file_name,'_model_a_',layer_name,'_split.mat'];
       file_big_model_short=[path_tree_junk,tree_model_file_name,'_model_a_',layer_name];
        
        %     load(file_big_model,'model_all')
        model_all=parload_model(file_big_model);
        
        nbasin=max(nbasins_use);
        
        
        
        for ibasin=1:nbasin
            file_big_model=[file_big_model_short,'basin_',num2str(ibasin),'.mat'];
        %     load(file_big_model,'model_all')
            
            if exist(file_big_model,'file')
            ...
            if ~isempty(Mdl)
               ...
               %             h.XTickLabel =PredictorNames;
                ...
                TreeJunk(ibasin,1).PredictorNames=PredictorNames;
                TreeJunk(ibasin,1).PredictorImpotance=imp;
                TreeJunk(ibasin,1).oobError=oobErrorBaggedEnsemble;
                TreeJunk(ibasin,1).LayerName=layer_name;
                TreeJunk(ibasin,1).LayerTitle=layer_title;
                TreeJunk(ibasin,1).BasinName=model_all(ibasin).name;
...
function [ModelTree]=parload_model(filename)
