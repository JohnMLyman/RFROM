function []=make_model_stats_all_years_seasonal_anom_split_orca_sal(TreeSetUp)
...
nbasin=length(TreeJunk);
 
for ibasin=1:nbasin
            TreeStats(ibasin,ilayer).PredictorNames=TreeJunk(ibasin).PredictorNames;
            TreeStats(ibasin,ilayer).PredictorImpotance=TreeJunk(ibasin).PredictorImpotance;
            TreeStats(ibasin,ilayer).oobError=TreeJunk(ibasin).oobError;
            TreeStats(ibasin,ilayer).LayerName=TreeJunk(ibasin).LayerName;
            TreeStats(ibasin,ilayer).LayerTitle=TreeJunk(ibasin).LayerTitle;
            TreeStats(ibasin,ilayer).BasinName=TreeJunk(ibasin).BasinName;
end
