...
file_name_argo='pfloat_sal_greg_Jan_2023_QC'

file_name='argo_2020_10_14_QC_argo_only_2005';
file_name='argo_2023_1_02_QC_argo_only_2005';
path_OHCA_data_out='C:\data\OHCA\'
path_OHCA_data_in='C:\OHCA\'
path_ohu_norm='C:\Users\jlyma\OneDrive - University of Hawaii\data\CERES\';
...
max_year_maps=2022.5;
min_year_maps=2002.5;
%set paths
...
time_grid=[min_year_maps-.5:.5:max_year_maps+.5];
...
file_name_out=[hdata_file_name,num2str(time_grid(1)-.5),'_',num2str(time_grid(end)-.5)];
...
lat_ht=lat;
% time_grid=time;
[LONht,LATht,TIMEht]=ndgrid(lon_ht,lat_ht,time_grid);
...
tpx_curve=zeros(nyears,1);
ht_curve=tpx_curve;
htone_curve=tpx_curve;
% tree_curve=tpx_curve;
httpx_curve=tpx_curve;
httpx_curve_north=tpx_curve;
httpx_curve_south=tpx_curve;

% httree_curve=tpx_curve;

% tree_out=nans(nlon_tpx,nlat_tpx,nyears);

[~,LATnorth,~]=ndgrid(lon_tpx,lat_tpx,tgrid);
pos_north=LATnorth >= 0;
...
%     htdiff_tree_out=Fhtdiff_tree(LON,LAT,TIME);
    ...
    httpx_out_south=httpx_out;
    httpx_out_north=httpx_out;

    httpx_out_south(pos_north)=nan;
    httpx_out_north(~pos_north)=nan;
...
%     httree_out=htdiff_tree_out+tree_out;
    ...
    jht_curve=nansum(ht_out.*arwj);
    jht_curve=squeeze(nansum(jht_curve));
    ht_curve=ht_curve+jht_curve;

    jone_curve=nansum(one_out.*arwj);
    jone_curve=squeeze(nansum(jone_curve))./layer_area;
    htone_curve=htone_curve+jht_curve./jone_curve;
    
        

    jhttpx_curve=nansum(httpx_out.*arwj);
    jhttpx_curve=squeeze(nansum(jhttpx_curve));
    httpx_curve=httpx_curve+jhttpx_curve;
    
    jhttpx_curve_north=nansum(httpx_out_north.*arwj);
    jhttpx_curve_north=squeeze(nansum(jhttpx_curve_north));
    httpx_curve_north=httpx_curve_north+jhttpx_curve_north;

    jhttpx_curve_south=nansum(httpx_out_south.*arwj);
    jhttpx_curve_south=squeeze(nansum(jhttpx_curve_south));
    httpx_curve_south=httpx_curve_south+jhttpx_curve_south;



    jtpx_curve=nansum(tpx_out.*arwj);
    jtpx_curve=squeeze(nansum(jtpx_curve));
    tpx_curve=tpx_curve+jtpx_curve;
...
good_ascii=find(tgrid>=2005 & tgrid <=max_year_maps);
tgrid=tgrid(good_ascii);
httpx_curve_north=httpx_curve_north(good_ascii);
httpx_curve=httpx_curve(good_ascii);
httpx_curve_south=httpx_curve_south(good_ascii);

write_ascii_norm_half(httpx_curve_north,tgrid,[path_ohu_norm,'combined_0_2000_northern_argo_only_sla.txt']);
write_ascii_norm(httpx_curve,tgrid,[path_ohu_norm,'combined_0_2000_whole_argo_only_sla.txt']);
write_ascii_norm_half(httpx_curve_south,tgrid,[path_ohu_norm,'combined_0_2000_southern_argo_only_sla.txt']);