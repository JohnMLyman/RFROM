function [model_all]=make_trees_mean_sal_nosst_nolatlon(use,ht_use,temp_use,tpx,layer_offset,coords,yr,nbasins_use,good_yr,good_prof)
...
for ibasin=nbasins_use
       ...
       if ~isempty(ht_mat)
           input_mat=nans(length(ht_mat),3);
           input_mat(:,1)=yr(good);
...
%            end
           ...
           input_mat(:,2)=tpx(good);
%            input_mat(:,5)=sst(good);
           input_mat(:,3)=temp_use(good);
