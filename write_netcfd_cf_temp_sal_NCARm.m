function []=write_netcfd_cf_temp_sal_NCARm(ptemp,sal,ssh,time,time_leap,lon,...
...
%     mean_depth_bnds_dimen(2).Length=ndepth;
    ...
    mySchema.Variables(1).Dimensions=map_dimen(1:2);
    ...
    mySchema.Variables(2).Dimensions=map_dimen(1:2);
    ...
    % time leap
    time_leap_att(1).Name='units';
    time_leap_att(1).Value='days since 0000-01-01 00:00:00';
    time_leap_att(2).Name='description';
    time_leap_att(2).Value='end day of 5-day average';
    time_leap_att(3).Name='standard_name';
    time_leap_att(3).Value='time';
    time_leap_att(4).Name='calendar';
    time_leap_att(4).Value='with leap years';

    mySchema.Variables(4).Name='time_leap';
    mySchema.Variables(4).Dimensions=map_dimen(4);
    mySchema.Variables(4).Attributes=time_leap_att;
    ...
    temp_att(2).Value=' Potential Temperature';
    
    mySchema.Variables(7).Name='ptemp';
    mySchema.Variables(7).Dimensions=map_dimen;
    mySchema.Variables(7).Attributes=temp_att;
   ...
   ssh=single(ssh);
    ...
    ncwrite(file,'latitude',latitude,[1 1]);
    ncwrite(file,'longitude',longitude,[1 1]);
    ncwrite(file,'time',time,[1]);
    ncwrite(file,'time_leap',time_leap,[1]);
    ncwrite(file,'depth',depth,[1]);
    ncwrite(file,'dz',dz,[1]);
    ncwrite(file,'ptemp',ptemp,[1,1,1,1]);  
    ncwrite(file,'sal',sal,[1,1,1,1]);
    ncwrite(file,'ssh',ssh,[1,1,1]);  
