...
path_nc_erddap=[path_ERDDAP,'netcdf_stable\'];
...
%%
 ...
 % put into the proper coordinates
    ...
    min_val=34;
    max_val=37.5;
v=VideoWriter([path_figs,'tree_sal_stable.avi']);
v.FrameRate = 10;
open(v)
...
filename_nc= [path_nc_erddap,'RFROM_JUNK_',num2str(iyear),'_0',num2str(imonth),'.mat'];

% [~,lon_tpx,lat_tpx,pres,~]=load_sal_nc(filename_nc);
 [~,lon_tpx,lat_tpx,pres]=load_sal_mat(filename_nc);
depth=pres;

for imonth=1:12
%      if imonth>=10
%               filename_nc= [path_nc_erddap,'RFROM_JUNK_',num2str(iyear),'_',num2str(imonth),'.nc'];
%            else
              filename_nc= [path_nc_erddap,'RFROM_JUNK_',num2str(iyear),'_0',num2str(imonth),'.mat'];
%       end
    
    [sal,~,~,~]=load_sal_mat(filename_nc);
    s_sal=size(sal);
...
function [sal_out,lon,lat,pres]=load_sal_mat(filename)
...
load(filename,'sal_out','lon','lat','pres')
