...
% end
% time_year_aviso=floor(time_aviso);
% time_mon_aviso=floor((time_aviso-time_year_aviso)*12)+1;

% mean_ht=nanmean(ht_estimate,3);
% good_s=time_year_aviso==syear&time_mon_aviso==smonth;
% map_s=nanmean(ht_estimate(:,:,good_s),3)-mean_ht;
% good_f=time_year_aviso==fyear&time_mon_aviso==fmonth;
...
file_f='RG_ArgoClim_202001_2019.nc';
file_s='RG_ArgoClim_202002_2019.nc';

path_file= 'C:\Users\jlyma\OneDrive - University of Hawaii\data\Roemmich_Gilson_Clim\';
file_name=[path_file,file_s];

time=ncread(file_name,'TIME');
lat_tpx=ncread(file_name,'LATITUDE');
lon_tpx=ncread(file_name,'LONGITUDE');
...
delta_p_total=repmat(delta_p_total,s(1),s(2),1);
temp=temp.*delta_p_total;
map_s=squeeze(nansum(temp,3)).*sw_dens(34.7,6,0).*sw_cp(34.7,6,0);
...
%     save 'OHCA_2019_tpx.mat' lon lat corrhc
    ...
    hold on
    m_coast('patch',[1 1 1]);
    t1=m_text(30,100,['(a) ', num2str(syear),',',num2str(smonth)],'fontsize',12);
    ...
    corrhc=(map_s-map_f)./((60*60*24*365.5));
    %%
    lon=lon_tpx';
    lat=lat_tpx';
    ...
    hold on
    m_coast('patch',[1 1 1]);
    t1=m_text(30,100,['(b) ' num2str(syear) ',' num2str(smonth) ' minus ' num2str(fyear) ',' num2str(fmonth)],'fontsize',12);
...
%  greg_map_hc_20_year_tree_scatter_plot_oco_tuna
...
%     set(jc,'XAxisLocation','bottom','pos',jcpos-[-.075 .805 .15 0])
    ...
    eval(['print -dpng -r600 -f1 ',path_figs,'oco_temp_gilson_publish_',num2str(year_of_oco_pub),'_for_',num2str(fyear),'_',num2str(fmonth)])
