function [ht_estimate]=bagged_yearly_tree_temp_SSH_SST_fast_1deg(iyear_mod,...
                time_aviso,ssh_total,sst_total,...
                nfiles,ht_estimate,TreePredictInfo,file_big_model_short,large_scale,tree_type) 


scale_box_deg=TreePredictInfo.scale_box_deg;
            ...
            if ibasin==2
              ...
              switch large_scale
                   case 'a'
                       ...
                       input_mat(:,3)=scale_box_deg.*floor(jlat(good)./scale_box_deg);
                       
                   case 'b'
                       input_mat(:,2)=scale_box_deg.*floor(jj_lon./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*round(jlat(good)./scale_box_deg);
                ...
                switch large_scale
                   case 'a'
                       input_mat(:,2)=scale_box_deg.*floor(jlon(good)./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*floor(jlat(good)./scale_box_deg);
                   case 'b'
                       input_mat(:,2)=scale_box_deg.*floor(jlon(good)./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*round(jlat(good)./scale_box_deg);
                   case 'c'
                       input_mat(:,2)=scale_box_deg.*round(jlon(good)./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*floor(jlat(good)./scale_box_deg);
                   case 'd'
                      input_mat(:,2)=scale_box_deg.*round(jlon(good)./scale_box_deg);
                       input_mat(:,3)=scale_box_deg.*round(jlat(good)./scale_box_deg);
