function []=multi_write_erddap_file_withcycle_nomean_v22_realtime(TreeSetUp)
...
for year_load=end_year_ssh
    ...
    for imonth=1:12
        ...
        if ~isempty(time_1950)
            ...
            if imonth>=10
                  file_name_nc= [path_nc_erddap,file_prefix,num2str(year_load),'_',num2str(imonth),'_REALTIME.nc'];
               else
                  file_name_nc= [path_nc_erddap,file_prefix,num2str(year_load),'_0',num2str(imonth),'_REALTIME.nc'];
            end
            if var_type =='t'
            ...
            elseif var_type=='s'
                break ;'NOT WRITTEN FOR SALINITY'
                 write_netcfd_cf_sal_pressure_mon_single(mon_estimate,time_1950,lon_tpx,...
                       lat_tpx,mean_pressure,mean_pressure_bnds,file_name_nc)
            elseif var_type=='h'
                write_netcfd_cf_heat_depth_mon_single_nomean_realv22(mon_estimate,time_1950,lon_tpx,...
