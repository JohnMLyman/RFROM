function [data_out]=find_woce_abssal_new_layers_1_tuna(data_in,coords,layer_bounds,path_OHCA_data_out)
...
%             j
     ...
     data_out{j,1}=temp_junk(good_pos);
     data_out{j,2}=sal_junk(good_pos);
     data_out{j,3}=press_junk(good_pos);



        if mod(j,1000)==0 ,disp(num2str([j,toc./60])), end
      end
