     ...
     if ~isempty(M)
        ...
        jw=repmat(weight_cross(pos_2d),nfiles,1);
            ...
            if ibasin==2
              ...
              switch large_scale
                  case 'a'
                       input_mat(:,3)=scale_box_deg_lat.*floor(jj_lat(good)./scale_box_deg_lat);
                   ...
                   case 'b'
                       input_mat(:,3)=scale_box_deg_lat.*round(jj_lat(good)./scale_box_deg_lat);
                       %compute the longitudnal scale
                       A=scale_box_deg_lat.*abs(sind(lat_change)./(cosd(lat_change).*sind(input_mat(:,3))));
                       B=scale_box_deg_lat./cosd(input_mat(:,3));
                       AA=min(A,scale_box_eq);
               ...
               switch large_scale
                   case 'a'
                       ...
                       input_mat(:,2)=scale_box_deg_lon.*floor(jj_lon(good)./scale_box_deg_lon);
                   case 'b'
                   ...
                   case 'c'
                       ...
                       input_mat(:,2)=scale_box_deg_lon.*round(jj_lon(good)./scale_box_deg_lon);
                   case 'd'
