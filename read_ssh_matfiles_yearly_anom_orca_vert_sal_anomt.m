function read_ssh_matfiles_yearly_anom_orca_vert_sal_anomt(TreeSetUp)
...
%         file_old=[path_new_tree,tree_file_name_out,'_split_7day.mat'];
%         FileInfo=dir(file_old);
%         [Y, M, D, H, MN, S] = datevec(FileInfo.datenum);
%         if M<9 || (M==9 && D<10) % only compute for months less then september 10th
...
%         else
%            
%              [ht_estimate_old,lon_tpx, lat_tpx,time_aviso]=parload_tree_year(file_old);
%         end
    end
...
function [ht_estimate,lon_tpx, lat_tpx,time_aviso]=parload_tree_year(filename)
         
         load (filename,'ht_estimate_year','lon_tpx', 'lat_tpx','time_aviso')

         ht_estimate=single(ht_estimate_year);
