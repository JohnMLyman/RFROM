function [ht_estimate]=bagged_yearly_tree_temp_SSH_vert_sal(iyear_mod,...
                time_aviso,ssh_total,temp_use,...
                nfiles,ht_estimate,ht_estimate_old,TreePredictInfo,...
                file_big_model,large_scale,tree_type) 
...
for ibasin=nbasins_use
 ...
 if ibasin<= n_mod_basin
     ...
     if ~isempty(M)
        ...
        jhtvert=double(ht_estimate_old(pos_3d));
        jtemp=double(temp_use(pos_3d));
        ...
        jw=repmat(weight_cross(pos_2d),nfiles,1);
            ...
            good=isfinite(jyr) & isfinite(jssh)&good_yr&isfinite(jhtvert);
            pos_use=find(pos_3d);
            pos_3d_use=pos_3d;
            ...
            if ibasin==2
               ...
               switch large_scale
                     ...
                     case 'a'
                       ...
                       input_mat(:,3)=5.*floor(jlat(good)./5);
                       
                   case 'b'
                       input_mat(:,2)=5.*floor(jj_lon./5);
                       input_mat(:,3)=5.*round(jlat(good)./5);
               ...
               switch large_scale
                   case 'a'
                       input_mat(:,2)=5.*floor(jlon(good)./5);
                       input_mat(:,3)=5.*floor(jlat(good)./5);
                   case 'b'
                       input_mat(:,2)=5.*floor(jlon(good)./5);
                       input_mat(:,3)=5.*round(jlat(good)./5);
                   case 'c'
                       input_mat(:,2)=5.*round(jlon(good)./5);
                       input_mat(:,3)=5.*floor(jlat(good)./5);
                   case 'd'
                      input_mat(:,2)=5.*round(jlon(good)./5);
                       input_mat(:,3)=5.*round(jlat(good)./5);
            ...
            input_mat(:,5)=jhtvert(good);
            input_mat(:,6)=jtemp(good);
