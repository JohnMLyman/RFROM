...
%   if exist(sal_file,'file');load(sal_file);oclnum_sal=oclnum;end
  ...
  [coords,dt,time,nprof,qual,fflag,ptype,pinfo,oclnum,temp]=read_EN3_file_1(ifile,path_EN4_in);
...
%                 % pick out good data points, sort and interpolate
           ...
           if length(ii)>2&&max_depth_junk>=100&&~isnan(xcor)&&coords(j,2)>=-80
                     ...
                     % correct XBT fall rate with Josh's old correction
                         ...
                         sal_junk=nans(length(temp_junk),1);
            ...
            sal_junk2=[];
                ...
                sal_junk=ones(length(press_junk),1).*35.0;
                        ...
                        for ilayer=2:length(layer_bounds)
                             ...
                             eval(['heat_',num2str(layer_bounds(ilayer-1)),'_',num2str(layer_bounds(ilayer)),'=NaN;'])
                             eval(['heat_0_',num2str(layer_bounds(ilayer)),'=NaN;'])
                             eval(['heat_junk=heat_',num2str(layer_bounds(ilayer-1)),'_',num2str(layer_bounds(ilayer)),';'])
                             eval(['heat_junk2=heat_0_',num2str(layer_bounds(ilayer)),';'])
                             if max_depth_junk >= layer_bounds(ilayer)
                                 
                                [heat_junk]=compute_depth_heat_depth_range_teos10_2016(data,heat_junk,coords(j,:),layer_bounds(ilayer-1),layer_bounds(ilayer));
                                [heat_junk2]=compute_depth_heat_depth_range_teos10_2016(data,heat_junk2,coords(j,:),0,layer_bounds(ilayer));
                            
                             end
                              eval(['heat_',num2str(layer_bounds(ilayer-1)),'_',num2str(layer_bounds(ilayer)),'=heat_junk;'])
                              eval(['heat_0_',num2str(layer_bounds(ilayer)),'=heat_junk2;'])
                         end
                        ...
                        for ilayer=2:length(layer_bounds)
                            ...
                            if ilayer~=length(layer_bounds)
                                ...
                                if_statement_name=[if_statement_name,'isfinite(heat_',num2str(layer_bounds(ilayer-1)),...
...
%                         if isfinite(heat_0_40) || isfinite(heat_40_90) || isfinite(heat_90_190) || ...
%                                 isfinite(heat_190_290) || isfinite(heat_290_450) || isfinite(heat_450_700) || isfinite(heat_700_950) || ...
%                                 isfinite(heat_950_1450) || isfinite(heat_1450_1950) ||  isfinite(heat_1950_2000) 
                        ...
                        mdep_wod(iprof)=max_depth_junk;
                         ...
                         for ilayer=2:length(layer_bounds)
                             ...
                             eval(['heat_',num2str(layer_bounds(ilayer-1)),'_',num2str(layer_bounds(ilayer)),'_wod(iprof)=',...
