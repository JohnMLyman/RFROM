...
for i1=1:o
    temp_to_W_per_dbar(:,:,i1)=1e4/(365.25*24*3600)*cp0*surf_area./(sw_g(-sw_dpth(prs(i1)+0*y,y),y));
    ju2=squeeze(ct_mod_coeffs(:,:,i1,2));
    ju1=squeeze(ct_mod_coeffs(:,:,i1,1));
    ju3=squeeze(temp_to_W_per_dbar(:,:,i1));
    % ii=find(isfinite(ju)==1&y>10&y<20&x>300&x<340);
    ...
    hc_trnd(:,i1)=nansum(ju2.*squeeze(temp_to_W_per_dbar(:,:,i1)),1);
    hc_trnd_atl(:,i1)=nansum(ju2.*atl_mask.*squeeze(temp_to_W_per_dbar(:,:,i1)),1);
    hc_trnd_pac(:,i1)=nansum(ju2.*pac_mask.*squeeze(temp_to_W_per_dbar(:,:,i1)),1);
    hc_trnd_ind(:,i1)=nansum(ju2.*ind_mask.*squeeze(temp_to_W_per_dbar(:,:,i1)),1);
    hc_trnd_med(:,i1)=nansum(ju2.*else_mask.*squeeze(temp_to_W_per_dbar(:,:,i1)),1);
    ii=find(isfinite(ju2)==1);
