function   tree_movie_split_temp_al_newl_eq(TreeSetUp)
%%
...
nlayer=length(layer_bounds);
 ...
 file_sum_name=[path_tree,tree_model_file_name,'_eq_all_press.mat'];
...
[~,pos_lat]=min(abs(lat_tpx));
% [~,pos_lon_topo]=min(abs(lon_topo-335));
bottom_depth=topo_tpx_new(:,pos_lat);
...
%     load([path_tree,tree_file_name,'_split_7day.mat'], 'ht_estimate','time_aviso');
  ...
  [ht_estimate,~]=load_htestimate([path_tree,tree_file_name,'_split_7day.mat']);
   ...
   ht_out_total(:,ilayer-1,:)=ht_estimate(:,pos_lat,:)+...
      ht_cycle(:,pos_lat,:)+ht_mean(:,pos_lat);
   ht_out_mean(:,ilayer-1)=ht_mean(:,pos_lat);
...
save(file_sum_name,'ht_estimate_mean','ht_out_mean','depth','lon_tpx','time_aviso','-v7.3')
...
%%
    ...
    load(file_sum_name,'ht_estimate_mean','ht_out_mean','lon_tpx','time_aviso','depth')
end
...
%%
 ...
 % put into the proper coordinates
    ...
    min_val=5;
    max_val=30.;
v=VideoWriter([path_figs,'tree_eq_top_press.avi']);
v.FrameRate = 10;
open(v)
...
%     corrhc=ht_estimate(:,:,iframe)./1e9;
   ...
   corrhc=ht_estimate_mean(:,:,iframe);
    ...
    [cs1,h1]=contourf(lon_tpx,-1.*depth,corrhc',...
        [-1000,min_val:del_cont:max_val]);
    a=gca;
    set(a,'xtick',[],'ytick',[])
        set(a,'tickdir','out','xtick',[0:10:360],'ytick',[-500:20:0],...
            'XAxisLocation','bottom');
    yticklabels(([500:-20:0]))
    axis(a,[0 360 -500 0])
    apos=get(a,'pos');
   title(['eq on ',num2str(time_aviso(iframe))])
