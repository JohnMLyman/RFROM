...
nlayers=length(layer_bounds);
 ...
 file_sum_name=[path_tree,tree_model_file_name,'_a16_error.mat'];

if ~exist(file_sum_name,'file')
    bagged_tree_ohca_maps_7_day_split_temp_error

 else
    load(file_sum_name,'ht_error','lat_tpx','time_aviso','depth')
end
...
v=VideoWriter([path_figs,'tree_a16_error.avi']);
v.FrameRate = 10;
open(v)
...
%     corrhc=ht_estimate(:,:,iframe)./1e9;
   ...
   corrhc=ht_error(:,:,iframe);
    ...
    axis(a,[-62 70 -2000 0])
    apos=get(a,'pos');
   title(['error A16 on ',num2str(time_aviso(iframe))])
