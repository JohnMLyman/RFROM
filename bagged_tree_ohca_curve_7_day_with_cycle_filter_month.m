...
for ilayer=startlayer:endlayer
    ...
    load([path_tree,tree_file_name_old,'_seasonal_cycle_expand.mat'],...
        'ht_cycle')
 %% remove the trend. smooth the ht_estimate. add the trend back
     
%  ht_estimate=ht_estimate-ht_trend;
      
        for ilon=1:nlon_tpx
            for ilat=1:nlat_tpx
                junk=squeeze(ht_estimate(ilon,ilat,:));
                good=isfinite(junk);
                if length(find(good))>3
                    junk=smooth(junk(:),10,'rloess');
                    junk(~good)=nan;

                    ht_estimate(ilon,ilat,:)=junk;
                end
...
save ([path_tree,'test_tree_curve_yearly_7day_2000_yearly_new_cycle_filter_month.mat'], 'tgrid', 'ht_curve', 'basin_curve', 'layer_curve');
