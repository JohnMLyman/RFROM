...
ht_gilson_year_all=smooth(ht_gilson_no_cyce,24,'rloess');
ht_gilson_year_all=smooth(ht_gilson_no_cyce,12);
time_gilson_rate_year=.5.*(time_gilson(1:end-end_ind_off_year)+time_gilson(start_ind_year:end));

rate_gilson_year_all=(ht_gilson_year_all(start_ind_year:end)-ht_gilson_year_all(1:end-end_ind_off_year)).*fac./12;
...
ht_tree_all_res_qu=smooth(ht_tree_all_res,4*3);
...
ht_year_all=smooth(ht_all_no_cyce,104,'rloess');
 ht_year_all=smooth(ht_all_no_cyce,52);
% ht_year_all=ht_year_all(1:26:end);
...
rate_qu_all=(ht_qu_all(start_ind_mon:end)-ht_qu_all(1:end-end_ind_off_mon)).*fac./4;
rate_qu_all_no_cycle=(ht_qu_all_no_cycle(start_ind_mon:end)-ht_qu_all_no_cycle(1:end-end_ind_off_mon)).*fac./4;
rate_year_all=(ht_year_all(start_ind_year:end)-ht_year_all(1:end-end_ind_off_year)).*fac./52;
% rate_year_all2=(ht_year_all2(start_ind_year:end)-ht_year_all2(1:end-end_ind_off_year)).*fac./52;

time_rate_tree_all=.5.*(time_tree_all(1:end-end_ind_off_mon)+time_tree_all(start_ind_mon:end));
...
plot(toa_time,toa,'k')
hold on
plot(time_rate_tree_all,rate_qu_all,'b')
plot(time_gilson_rate,gilson_rate_qu,'r')
...
hold on
plot(time_rate_tree_all,res_rate_qu_all,'b')
plot(time_gilson_rate,res_rate_qu_gilson,'r')
...
plot(time_gilson,smooth(ht_gilson_no_cyce,12)./1e21,'r')
hold on
plot(time_tree_all,smooth(ht_mon_all_no_cyce,52)./1e21,'k')
...
% plot(time_gilson,ht_curve_gilson-nanmean(ht_curve_gilson))
good_gilson=find(time_gilson>=2008);
good_tree=find(time_tree_all>=2008);
offset_gilson=nanmean(ht_gilson_no_cyce(good_gilson));
offset_tree=nanmean(ht_mon_all_no_cyce(good_tree));

plot(time_gilson,(ht_gilson_no_cyce-offset_gilson)./1e21,'r')
hold on
...
axis([min_year max_year min_ohca max_ohca])
ylabel('OHC (ZJ)','Fontsize',16,'Fontname','Arial')
...
% plot(tgrid,ht_curve-nanmean(ht_curve))
figure
plot(time_rate_tree_all_year,rate_year_all,'k')
hold on
plot(time_rate_tree_all,smooth(rate_qu_all_no_cycle,52),'r')
% plot(time_gilson_rate_year,rate_gilson_year_all,'r')
load ceres_global_toa_6mon.mat ceres_global_rate_6mon ceres_global_time_6mon
plot(ceres_global_time_6mon,ceres_global_rate_6mon,'b')

figure
plot(toa_time,smooth(res_rate_toa,12))
hold on
plot(time_rate_tree_all_year,rate_year_all-.6,'k')
...
plot(toa_time_yearly,toa_yearly)
hold on

time_ohc_yearly=time_rate_tree_all_year(pos_new_ohc);
rate_ohc_yearly=rate_year_all(pos_new_ohc)
