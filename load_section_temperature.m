function [lat_tpx,depth,ht_section,ht_section_wc]=load_section_temperature(TreeSetUp,iyear)
...
s=size(ht_estimate_year);
[~,pos_lon]=min(abs(lon_tpx-220));
ht_section=nan(s(2),nlayer-1,s(3));
ht_section_wc=ht_section;
...
%         tree_file_name_out_withoutcycle=[tree_model_withoutcycle,'_',layer_name,'_',num2str(year_load)];
        ...
        ht_section_wc(:,ilayer-1,:)=ht_estimate_year_wc(pos_lon,:,:);
        ht_section(:,ilayer-1,:)=ht_estimate_year(pos_lon,:,:);
