...
% ht_gilson_res_year=smooth(ht_gilson_res,24,'rloess');
...
% ht_tree_all_res_mon=smooth(ht_tree_all_res,8,'rloess');
% ht_tree_all_res_qu=smooth(ht_tree_all_res,26,'rloess');
% ht_tree_all_res_semi=smooth(ht_tree_all_res,52,'rloess');
% ht_tree_all_res_year=smooth(ht_tree_all_res,104,'rloess');
...
rate_year_all_no_cycle_mon=(ht_year_all_no_cycle(start_ind_mon:end)-ht_year_all_no_cycle(1:end-end_ind_off_mon)).*fac./4;
...
% 
% toa_yearly=smooth(res_rate_toa,24,'rloess');
% toa_qu=smooth(res_rate_toa,6,'rloess');
...
text(2009,-30,['RG ratio= ',num2str(round(frac_gilson_all))],'color',mycor(4,:),'fontsize',8)
text(2009,-35,['RFOM ratio= ',num2str(round(frac_tree_all))],'color','k','fontsize',8)
text(2008.5,36,'(a)')
...
%   text_del=text_del*10;
  ...
  text_size=12;
  text_off=-50;
text(year_text,text_off+text_del*4,'RG','color',mycor(4,:),'FontName','Arial','FontSize',text_size)
plot([year_text-2 year_text-1],[text_off+text_del*4 text_off+text_del*4],'color',mycor(4,:),'linewidth',2)

text(year_text,text_off+text_del*3,'RFOM','color','k','FontName','Arial','FontSize',text_size)
...
hold on
% plot(time_rate_tree_all_mon(plot_pos_tree_mon),rate_year_all_no_cycle_mon(plot_pos_tree_mon),'g','LineWidth',1.25)
hold on
...
set(gca,'tickdir','out','Fontname','Arial','box','on')
text(2009,-.5,['RG ratio= ',num2str(round(frac_gilson_year))],'color',mycor(4,:),'fontsize',8)
text(2009,-.7,['RFOM ratio= ',num2str(round(frac_tree_year))],'color','k','fontsize',8)
text(2008.6,1.7,'(b)')
ylabel('W m^{-2}','Fontsize',10,'Fontname','Arial')
...
path_figs='C:\data\OHCA\figs\tree_paper\'
 eval(['print -dpng -r600 -f1 ',path_figs,'toa_tree_gilson_ceres2'])
%% yearly
...
%   text_del=text_del*10;
  ...
  text_size=12;
  text_off=-15;
text(year_text-.95,text_off+text_del*4,'RG','color',mycor(4,:),'FontName','Arial','FontSize',text_size)
plot([year_text-1.1 year_text-1],[text_off+text_del*4 text_off+text_del*4],'color',mycor(4,:),'linewidth',2)

text(year_text-.95,text_off+text_del*3,'RFOM','color','k','FontName','Arial','FontSize',text_size)
...
path_figs='C:\data\OHCA\figs\tree_paper\'
 eval(['print -dpng -r600 -f2 ',path_figs,'toa_tree_gilson_ceres_cycle_new'])
% 
...
d = Document([path_figs,'Tree_corr_table'],'docx');
append(d,mltableObj);
close(d);
% rptview(d);
...
import mlreportgen.dom.*
mltableObj = MATLABTable(mltable);
d = Document([path_figs,'RG_corr_table'],'docx');
append(d,mltableObj);
close(d);
% rptview(d);
